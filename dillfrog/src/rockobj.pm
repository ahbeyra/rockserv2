...
sub can_be_lifted {
    ...
    if($self->{'EQD'}) {
        ...
        return 0;
    } elsif ($self->{'MINLEV'} && $by->{'LEV'} < $self->{'MINLEV'}) {
...
sub dam_bonus {
    ...
    my $self = shift;
    return int( $self->{'STAT'}->[STR] + rand($self->{'WC'}) ); 
}
    ...
    if (!$html) {  # used to be if($self->{'TELNET'} == 1)
        ...
        if ($self->pref_get('stat prompt')) { 
            if (my $tmp = $self->{'PFMT'}) {
                ...
                $cap .= "{4}>{5}$self->{'HP'}/$self->{'MAXH'} {14}$self->{'MA'}/$self->{'MAXM'} {7}$self->{'T'}$main::afkpromptmap[$self->{'AFK'}]> {17}";
            }
        } else {
            $cap .= '{4}>> {17}';
        }
...
sub attack_melee
{
    ...
    if($self->{'TYPE'} == OTYPE_NPC) {
        $damcap = ($weapon->{'WC'} || ($self->{'LEV'}/10+1)) * 4.5;
    } else {
        $damcap = ($weapon->{'WC'} || 0) * 4.5;
    }
    ...
    if ($self->pref_get('brief combat descriptions')) {
        ...
        if (@pHits) {
            ...
            $cap = "{2}You {5}$weapon->{'FPSHD'} {2}$victim->{'NAME'} for " . join('{2}, ', @pHits) . "$last\{2}damage";
    ...
    if ($victim->pref_get('brief combat descriptions')) {  
        ...
        if (@pHits) { 
            ...
            $cap = "{1}$self->{'NAME'} {5}$weapon->{'TPSHD'} {1}you for {17}".join('{1}, ', @pHits)."$last\{1}damage";
...
sub inv_list {
    ...
    foreach my $obj (@a) {
        ...
        if ($obj->{'WORN'}) {
            $Wincount{$istr . $obj->{'NAME'} . ' {7}[' . $obj->{'ATYPE'} . ']'}++;
        } elsif ($obj->{'EQD'}) {
            $Wincount{$istr . $obj->{'NAME'} . ' {7}[{1}equipped{7}]'}++;
        } else {
            $incount{$istr . $obj->{'NAME'}}++;
        }
    }
...
{16}Lives: {17}%-15d {16}Armor Class: {17}%-10d  {16} Email Addy: {17}%-10s
...
END_CAP
    ...
    $cap .= sprintf($format_str, $self->{'NAME'},            $main::races[$self->{'RACE'}], $self->{'GENDER'}
                             , $self->{'LIVES'}, $self->{'AOFFSET'},            substr($self->{'EMAIL'}, 0, 9)
...
sub item_drop {
 ...
 if (lc($iname) eq 'all') {
   ...
   my $o; foreach $o ($self->inv_objs) { if ( $self->item_drop($o->{'NAME'}) == 0) { return(0); } }
   return 1;
 }
   ...
   if (index($str, ' about ') == -1) {
      ...
      if ( $success == 1 ) { 
		  ...
		  unless(defined($cap) || !defined($obj->{'REC'})) {
			  ...
			  my $sth = $dbh->prepare("SELECT response_type, response_text FROM $main::db_name\.on_ask_responses WHERE (item_id = -1 OR item_id = ?) AND ? REGEXP response_match ORDER BY item_id DESC, match_order DESC LIMIT 1");
			  ...
			  if ($row) {
			      ...
			      my $sthb = $dbh->prepare("SELECT response_match FROM $main::db_name\.on_ask_responses WHERE item_id = ? AND is_visible = 'Y'");
...
#         &main::rock_shout(undef, "{17}$self->{'NAME'} is spectral shroud for $obj->{'NAME'}\n", 1);
     ...
     if ($self->{'TYPE'}==1) { 
         ...
         foreach $obj (@inv_objs) {
             if ( $obj->{'LASTDROP'} || (rand(100) < ($obj->{'DRPPCT'}||4) ) ) { 
                 # if a player originally had the item or random,...
                 $obj->{'VAL'} = ceil ($obj->{'VAL'}*.7); # Don't let it hit 0
                 $obj->{'WC'} = int ($obj->{'WC'}*.8);
                 $obj->{'AC'} = int ($obj->{'AC'}*.8);
                 delete $obj->{'LASTDROP'};
                 push (@objnames,$obj->{'NAME'});
...
sub stats_update {
   ...
   for ($n=6; $n<=22; $n++) {
     ...
     elsif($self->{'STAT'}->[$n]>1000) { $self->{'STAT'}->[$n] = 1000; }
   }
   ...
   # as for our apparrel..
   my $wobj;
   if($self->{'WEAPON'} && ($wobj = $main::objs->{$self->{'WEAPON'}}) && ($wobj->{'WSTAT'} && @{$wobj->{'WSTAT'}}) ) { 
...
sub die {
    ...
    if($killer) {
        ...
        if( $self->{'TYPE'}==1 && $killer->{'TYPE'}==1 ){ 
            ...
            if ($killer->{'RACE'} == $self->{'RACE'}) {
                my $cap = "{7}## {7}$deathloc: {14}Sadly {12}Rejoice!.. {2}\{$self->{'RACE'}\}$self->{'NAME'} {7}was done in by \{$killer->{'RACE'}\}$killer->{'NAME'}\{7}!\n";
                &main::rock_rshout($killer, $cap, 'silence deaths');
                $main::rock_talker->broadcast('R2SHOUT', $cap) if defined $main::rock_talker;
            } else {
                my $cap = "{7}## {7}$deathloc: {12}Rejoice! \{$self->{'RACE'}\}$self->{'NAME'} {7}was done in by \{$killer->{'RACE'}\}$killer->{'NAME'}\{7}!\n";
                my $cap_sorrow = "{7}## {7}$deathloc: {14}Sorrow.. \{$self->{'RACE'}\}$self->{'NAME'} {7}was slain by \{$killer->{'RACE'}\}$killer->{'NAME'}\{7}!\n";
                &main::rock_rshout($killer, $cap, 'silence deaths');
                &main::rock_rshout($self, $cap_sorrow, 'silence deaths');
                $main::rock_talker->broadcast('R2SHOUT', $cap) if defined $main::rock_talker;
            }
    ...
    unless ($pvproom) {
        ...
        $orb = $self->inv_rec_scan(135);
        if ($orb && !$main::map->[$self->{'ROOM'}]->{'!TURQ_ORB'}) {
            $self->log_append("{6}In a flash, the $orb->{'NAME'} encompasses your items.\n");
            $orb->obj_dissolve();
        } elsif ($self->{'ADMIN'}) {
            $self->log_error("Though you had no orbs, your adminness has caused you to keep your items.");
        } else {
            $self->inv_remove();
        }
 ...
 if(lc($iname) eq 'all') { 
    ...
    foreach my $i ($self->inv_objs) {
      if(defined($i->{'ATYPE'}) && !defined($self->{$i->{'ATYPE'}})) {
         if($self->item_hwear($i, 1)) { push(@ilist, $i->{'NAME'}); }
      }
    }
...
sub item_hequip {
  ...
  unless ($self->has_minstats($item->{'EQUIP_MINSTATS'})) {
      $self->log_error("You don't feel skilled enough to wield $item->{'NAME'}.");
	  return 0;
  }
...
sub item_hwear {
  ...
  unless ($self->has_minstats($item->{'EQUIP_MINSTATS'})) {
      $self->log_error("You don't feel skilled enough to wear $item->{'NAME'}.");
	  return 0;
  }

  ## Check if we can't wear that item.
  if(!($item->can_wear() && $item->{'ATYPE'})) { $self->log_append("{3}You cannot wear $item->{'NAME'}.\n"); return(0); }
  ## add item to apparel
...
sub can_wear { my $self = shift; return(1); }
...
sub apparel_update {
  ...
  if($self->{'APRL'} && @{$self->{'APRL'}}) { $self->arr_rotate(\@{$self->{'APRL'}}); }
...
sub obj_dump {
   ...
   if($self->{'NAME_CHANGE'} && $logoff) { 
      ...
      my $dbh = rockdb::db_get_conn();
          my $sth = $dbh->prepare("UPDATE $main::db_name\.players SET NAME='$self->{'NAME_CHANGE'}\' WHERE NAME='$self->{'NAME'}\'");
          $sth->execute();
   ...
   if ($logoff && $self->{'DELETE_CHARACTER_UPON_LOGOUT'}) { 
      my $dbh = rockdb::db_get_conn();
      my $sth = $dbh->prepare("DELETE FROM $main::db_name\.players WHERE NAME='$self->{'NAME'}\'");
      $sth->execute();
      unlink(&main::insure_filename('./saved/'.lc($self->{'NAME'}.'.r2')));
...
sub un_portalize {
	...
	if((!$self->{'PORTAL'}) || !($main::portals->{$self->{'PORTAL'}} && @{$main::portals->{$self->{'PORTAL'}}}) ) { return; }
...
sub on_enter {
  ...
  if(!$self->{'PORTAL'} || $self->{'NOENTER'}) {
     ...
     my (@outlets, $oid);
     if (!($main::portals->{$self->{'PORTAL'}} && @{$main::portals->{$self->{'PORTAL'}}})) {
...
INSERT INTO $main::db_name\.daily_scores
...
#        $main::map->[$main::roomaliases{'arenahall'}]->inv_add($image);
     ...
     if($image->{'VERTIME'} < $main::ver_time) {  
     ...
     $image->up_general();
     if(      (!$image->{'NEWTURNS'} || (&main::day_get($image->{'NEWTURNS'}) ne &main::day_get(0)) || (($image->{'NEWTURNS'} + 60*60*24) < time)) ) { 
        #
       ...
       # }
        ...
        if($image->{'SOLDIER'} && $image->{'REPU'} >= 2) {
            ...
            if ($image->{'GENERAL'}) {
                ...
                my $bonus_turns = int(100 + 300 * $image->monoliths_captured() + rand(10 * $image->{'REPU'}));
                if ($bonus_turns > 0) {
        ...
        if($image->{'GIFT'}->{'ROAR'}) { $image->{'ROARS'}=5; }
        $image->{'VIGOR'}=1.5;
        $image->{'MAXHIT'} = 0;
...
sub maxinv_set {
   my $self = shift;
   $self->{'MAXINV'} = 15 + 2 * $self->skill_has(17) + 5 * $self->skill_has(20);
   return;
}
