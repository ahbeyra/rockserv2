...
INSERT INTO $main::db_name\.event_log
...
Lingua::Ispell::use_personal_dictionary("/usr/dillfrog/r2/dict/rock_dictionary.dat");
...
sub get_item_name_by_rec {
    ...
    my $item_rec = shift;
    my ($name) = sql_select_mult_row_linear("SELECT item_name FROM $main::db_name\.item_names_by_rec WHERE item_id = ?", abs int $item_rec);
    return $name;
}
...
sub check_new_turns {
   if (&main::day_get($main::rock_stats{'newturns'} || 10) ne &main::day_get(0)) {
       ...
       $main::rock_stats{'pvprestrict'} = $main::pvp_restrict = int rand 11;
...
sub obj_list {
    ...
    $searchstr = lc($_[0]);
    $cap = sprintf("{40}{3}%5s %4s %2s%-30s %4s\n", 'OBJID', 'ROOM', undef, 'OBJ. NAME', 'TYPE');
    $time = time; # only call it once
    foreach $key (sort by_number (keys(%{$main::objs}))) {
        $obj = $main::objs->{$key};
        if (index(lc($obj->{'NAME'}),$searchstr) != -1) { 
            $cap .= sprintf("{6}%5d %4d %2s{2}%-30s {3}%4s\n", $key, $obj->{'ROOM'}, $main::activemap{defined($main::activeusers->{$obj->{'OBJID'}})}, substr($obj->{'NAME'}, 0, 30), $main::typemap{$obj->{'TYPE'}});
        }
    }
...
sub object_briefing {
 ...
 for(my $n=0; $n<=$maxitem; $n++) {
   ...
   if($o->{'TYPE'}==0) {
     ...
     if($booststr) { $booststr = "*Worn*: $booststr"; }
   ...
   elsif($o->{'WC'}) { push(@weapons, &oBriefTitle(scalar @weapons).$itemcap); }
...
sub check_auto_cleanup {
    # Don't try cleaning up unless the user enabled it in mainconsts.bse or similar.
    # We skip this when only admins can log in, in case:
    #  1) They're testing something in the game, so they dont want reboots
    #  2) They're testing this cleanup code, and this way we prevent it from
    #     scheduling overlapping a 5-minute reboot (trust me, this happens..*cough*)
    return if !$main::do_cleanup || $main::admin_login_only;  

    # Check other reasons we shouldn't auto-cleanup.
    # 1) Don't reboot if at least N users are in the game
    ...
    if ($_[0] || ($users < 1 && ((time - $main::starttime) > 3600*15) && (time - $main::rock_stats{'lastautocleanup'} > 3600*12))) {
        ...
        # Only admins can log in during this 5-minute period
        $main::admin_login_only = 1;
        
        # Schedule the actual cleanup for 5 minutes from now
        $main::eventman->enqueue(60 * 5, \&main::auto_cleanup);
        
        # Just in case the cleanup fails, schedule admin-only login to be disabled
        # shortly after the reboot would have happened.
        $main::eventman->enqueue(60 * 5 + 30, sub { $main::admin_login_only=0; } );
        
        # Alert that the reboot's going down slightly before it does.
        $main::eventman->enqueue(60 * 5 - 1, \&main::rock_shout, undef, "{1}*** {16}Maintainance Routine Beginning...\n");
        
        # Fire reminders on the minute, and 30-sec before
        for(my $i=4; $i>0; $i--) { 
        ...
        # do other cleanuppy type stuff anyway?
        # nothing for now
    }
    
    # Regardless of whether we decided to reboot or not, check this function again
    # later (warning: If you call this check_auto_cleanup function manually you'll
    # end up creating multiple queued instances of this function, which could slow
    # your server down if used to the extreme.. etc.. but this is atypical for you
    # to do anyway).
...
sub auto_cleanup {
     ...
     # Ride on the commands.bse flavor (doesn't do everything we normally do here,
     # but at least it doesnt seem to crash for the same reason :-))
     &{$main::adminbase_sing->{'restart'}}();
...
sub write_course_html() { 
    ...
    $c .= "</TABLE>\n";
    $c .= "<br><B>&copy; 1999-2003 $main::owner_name. All Rights Reserved. DO NOT COPY WITHOUT WRITTEN PERMISSION. However, you may link to this document without consent.</B><BR>\n";
    $c .= "<!--#include virtual=\"/include/footer.asp\"-->";

    my $index_filepath = "$main::base_web_dir/help/courses/index.shtml";
...
sub write_playerotm_html() { 
    ...
    close(F);
    &main::rock_hbcastshout(4, undef, "{4}>>> {14}$playername {4}is the new player of the moment!\n");
