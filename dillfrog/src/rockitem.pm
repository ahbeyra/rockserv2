...
sub on_idle {
  ...
  if($self->{'CONTAINEDBY'}) {
    ...
    if($cby->inv_has($self) && $self->{'EQD'}) { 
       if($cby->{'REPU'}<100) { $cby->log_append("{16}You are not deserving of such compliments.\n"); $cby->item_hunequip(); }
       else {
...
sub on_use {
  ...
  &main::rock_shout(undef, "## ADMIN COOOOL! {11}$user->{'NAME'} used sphere to split up a group of: ".join(', ', map { $_->{'NAME'} } @$victim_set).".\n", 1);
...
sub on_wind {
  ...
  if (!$winder->is_developer() && !$winder->inv_has_rec(337) && !$winder->inv_has_rec(363)) {
...
#   
  ...
  for (my $n=1; $n<=2; $n++) {
    $spoa = ($main::map->[$main::roomaliases{'teama-itemsa'}]->item_spawn(192))->{'OBJID'}; $main::objs->{$spoa}->{'RACE'}=6;
    $spob = ($main::map->[$main::roomaliases{'teamb-itemsa'}]->item_spawn(192))->{'OBJID'}; $main::objs->{$spob}->{'RACE'}=7;
    $spoc = ($main::map->[$main::roomaliases{'teama-itemsa'}]->item_spawn(48))->{'OBJID'};  $main::objs->{$spoc}->{'RACE'}=6;
    $spod = ($main::map->[$main::roomaliases{'teamb-itemsa'}]->item_spawn(48))->{'OBJID'};  $main::objs->{$spod}->{'RACE'}=7;
    $spoe = ($main::map->[$main::roomaliases{'teama-itemsb'}]->item_spawn(195))->{'OBJID'}; $main::objs->{$spoe}->{'RACE'}=6;
    $spof = ($main::map->[$main::roomaliases{'teamb-itemsb'}]->item_spawn(195))->{'OBJID'}; $main::objs->{$spof}->{'RACE'}=7;
    $spoe = ($main::map->[$main::roomaliases{'teama-itemsb'}]->item_spawn(347))->{'OBJID'}; $main::objs->{$spoe}->{'RACE'}=6;
    $spof = ($main::map->[$main::roomaliases{'teamb-itemsb'}]->item_spawn(347))->{'OBJID'}; $main::objs->{$spof}->{'RACE'}=7;
    $spog = ($main::map->[$main::roomaliases{'teama-itemsa'}]->item_spawn(267))->{'OBJID'}; $main::objs->{$spoa}->{'RACE'}=6;
    $spoh = ($main::map->[$main::roomaliases{'teamb-itemsa'}]->item_spawn(267))->{'OBJID'}; $main::objs->{$spoa}->{'RACE'}=6;
 ...
 foreach my $o (@teama, @teamb) {
  ...
  foreach my $o (@teama) { 
      ...
      $o->{'TEMPRACE'} = $o->{'RACE'}; $o->{'RACE'}=6;    # Set temporary races
  ...
  foreach my $o (@teamb) {
      ...
      $o->{'TEMPRACE'} = $o->{'RACE'}; $o->{'RACE'}=7;    # Set temporary races
...
sub on_death_notify {
   ...
   if($killer->{'RACE'}!=$dyer->{'RACE'}) { 
    if($killer->{'RACE'}==6) { $self->{'AI'}->{'TEAMA'}->[1]++; }
    if($killer->{'RACE'}==7) { $self->{'AI'}->{'TEAMB'}->[1]++; }
   } else {
    if($killer->{'RACE'}==6) { $self->{'AI'}->{'TEAMA'}->[1]--; }
    if($killer->{'RACE'}==7) { $self->{'AI'}->{'TEAMB'}->[1]--; }
   }
...
sub on_room_enter (who, direction from) {
  ...
  if( ($main::allyfriend[$self->{'RACE'}]->[$who->{'RACE'}]) || ($who->{'TYPE'}<=0) || ($who->{'RACE'} == 1) || (rand(1) > $self->{'CATCHFUZ'}) ){ return; }
...
sub def_set {
  ...
  $self->{'MAXINV'}=40 unless ($self->{'MAXINV'}>40);
