          ...
          if($socket eq $listen) {
           ...
           #    eval { if(!defined($main::ip_resolved{$new->peerhost})) { system('nslookup '.quotemeta($new->peerhost()).' &'); } };
            ...
            #    eval { if(!defined($main::ip_resolved{$new->peerhost})) { system('nslookup '.quotemeta($new->peerhost()).' &'); } };
                ...
                $outq[$socket->fileno] .= &cmd_make_telnet(&TELNET_IAC, &TELNET_WONT, &TELOPT_ECHO, &TELNET_IAC, &TELNET_DO, &TELOPT_ECHO, &TELNET_IAC, &TELNET_WONT, &TELOPT_BINARY);
...
#                       print "Yep had second argument..\n";
    ...
    if (defined $main::rock_talker) {
        eval { $main::rock_talker->pump(); };
    }
...
sub handle_talker_line {
  ...
  my ($code, $msg) = $line =~ /^(\S+)\s*(.*)$/;
  return if !$code;

  return if $code =~ /^[A-Za-z-]+:/;
  return if $code !~ /^R2/;

  $main::talker_socks->{$fileno} = 1 if $main::talker_socks;

  if ($code eq 'R2SHOUT') {
    my ($from, $text) = talkserv_parse_shout($msg);
    talkserv_store_message($from, $text);
    return;
  }
...
sub talkserv_store_message {
  ...
  my $dbh = eval { rockdb::db_get_conn() };
  return if !$dbh;
  talkserv_ensure_schema($dbh);

  my $realm = 'dillfrog';
  my $table = $main::db_name . '.talk_messages';
  my $stmt = "INSERT INTO $table (realm, user_id, user_name, body, created_at) VALUES (?, ?, ?, ?, NOW())";
  eval { $dbh->do($stmt, undef, $realm, 0, $from, $text); };
  return;
}

sub talkserv_ensure_schema {
  my $dbh = shift;
  return if $main::talkserv_schema_ok;

  my $driver = lc($main::db_driver || 'mariadb');
  my $table = $main::db_name . '.talk_messages';
