...
&rockserv_apply_env_prefs();
&rockserv_apply_restart_notice();
&rockserv_heartbeat_init();
...
sub cleanup_objs {
  ...
  foreach $obj (values(%{$main::objs})) {
    ...
    if(!$obj->{'CONTAINEDBY'} && ($obj->{'TYPE'}>=0) && ("$obj->{'CONTAINEDBY'}" eq "") ) { print "Object $obj: $obj->{'NAME'} ($obj->{'OBJID'}) w/undef cby: $obj->{'CONTAINEDBY'}\n"; &rock_shout(undef, "{1}Object $obj: $obj->{'NAME'} ($obj->{'OBJID'}) w/undef cby: $obj->{'CONTAINEDBY'}\n", 1); }
