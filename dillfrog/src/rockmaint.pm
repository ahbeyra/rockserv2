...
INSERT INTO $main::db_name\.altwatch
...
#        &main::rock_shout(undef, "{2}:: {1}rokamaint{2} :: {17}Synching maintainstart to $main::maintainstart.\n", 1);
    } elsif( ($t - $self->{'@HIT'}) > ($self->{'@SYNC'}+10) ) { $main::maintainstart += 1; &main::rock_shout(undef, "{2}:: {1}rokamaint{2} :: {17}Synching maintainstart to $main::maintainstart.\n", 1); }
    # if full idle scheduled, do it.
    if ($self->{'IDLE'}<$t) { $self->idle_full(); $self->{'IDLE'}= time + 10*60   * 12; }
    # if rem_active_users scheduled, do it.
    if ($self->{'REMACTIVE'}<$t) { &main::rem_inactive_users; $self->{'REMACTIVE'}= time + 30; }
    # if spawn scheduled, do it.
    if($self->{'SPAWN'}<$t) {  &users_ahist_cleanup; &main::spawn_stuff; $self->{'SPAWN'}= time + 1.5*60; }
    # if user saving scheduled, do it.
    if($self->{'USAVE'}<$t) { $self->users_save; $self->{'USAVE'}= time + 14.3*60; }
    # check for privacy invasion every 3 min
    if($self->{'INVADE'}<$t) { &rockobj::invade_privacy; &rockobj::adminstats_commit; $self->{'INVADE'}= time + 2*60; }
    # compress descs
    if($self->{'COMPRESS'}<$t) { &main::compress_descs; $self->{'COMPRESS'}= time + 20*60; }
    # cleanup rooms
    if($self->{'CLROOMS'}<$t) { &main::cleanup_rooms; $self->{'CLROOMS'}= time + 2*60; }
    # cleanup rooms
    if($self->{'CLOBJS'}<$t) { &main::badobj_scan; &main::cleanup_objs(); $self->{'CLOBJS'}= time + 5*60; }
    # dual chars
    if($self->{'DUALCHAR'}<$t) { $main::dual_friend->save(); $self->{'DUALCHAR'}= time + 5.25*60; }
    # courses
    if($self->{'COURSEAWARD'}<$t) { &courses_award(); $self->{'COURSEAWARD'}= int( time + (int rand 10)+10 ); }
    # players' mana
    if($self->{'PMANA'}<$t) { &rooms_uncrowd; &mana_disperse(); $self->{'PMANA'}= int( time + (rand(1)+1.5)*60 ); }
    # "general" votes
    if($self->{'GENVOTES'}<$t) { &ora_scores::ora_players_count; &votes_tally(); $self->{'GENVOTES'}= int( time + (rand(10)+60)*60*3 ); } # every 3 hours or so
    
    if ($self->{'ARMAGEDDON'}<$t) { &rock_maint::update_armageddon_status(); $self->{'ARMAGEDDON'} = int ($t + (rand(1)+1.5)*60); }
    
    # &main::rock_flatten_realm;
    # clean up nullity
    #$main::map->[0]->cleanup_inactive;
    if(scalar %main::events) { &main::events_update; }
...
#           foreach my $n ($r->inv_pobjs) { $n->on_idle(); }
   ...
   &main::rock_shout(undef, "{17}-- Tried spreading NPCs thinner; affected $rcount rooms (@rooms_affected).\n", 1);
}
...
sub update_armageddon_status {
    ...
    # instantatnaennanenous
    $dbh->do("UPDATE $main::db_name\.game_server_settings SET value=? WHERE name='armageddon_started_by_race'", undef, $main::rock_stats{'armageddon_started_by_race'});
    $dbh->do("UPDATE $main::db_name\.game_server_settings SET value=? WHERE name='armageddon_is_possible'", undef, $main::rock_stats{'armageddon_is_possible'});
    
    my %monoliths_owned = rockdb::sql_select_mult_row_linear("select owned_by_race, COUNT(*) from $main::db_name\.monolith_capture_status GROUP BY owned_by_race");
    ...
    if (keys(%monoliths_owned) == 1) {
        ...
        my @partial_control_monoliths = rockdb::sql_select_mult_row_linear("select name from $main::db_name\.monolith_capture_status where date_captured > DATE_ADD(sysdate(), INTERVAL -$capture_hours HOUR)");
...
SELECT auction_id, high_bid, high_bid_uin, item_name, SELLER.userid
FROM $main::db_name\.auctions A, dillfrog.accounts SELLER
WHERE end_date < sysdate()
   AND returned_cryl = 'N'
   AND SELLER.uin = A.seller_uin
END_SQL
...
SELECT B.bid_id, B.max_bid, B.bidder_uin, A.userid 
FROM $main::db_name\.auction_bids B, dillfrog.accounts A
WHERE auction_id = ?
    AND B.bidder_uin = A.uin
END_SQL
		...
		my $winner_name = $winner_uin ? ucfirst((rockdb::sql_select_row("SELECT userid FROM dillfrog.accounts WHERE uin=?", $winner_uin))[0])
		                  : undef;
...
INSERT INTO $main::db_name\.rock_certs
(name, c_type, c_val)
VALUES
(?, 'K', ?)
END_SQL
...
END_SQL
#		$dbh->do(<<END_SQL, undef, $auction->[0]);
#DELETE FROM $main::db_name\.auction_bids
#WHERE auction_id = ?
#END_SQL
...
END_HTML
    ...
    open F, ">/var/www/html/games/r2/currplayers.shtml"
	    or  die "Could not open currplayer file: $!";
	my $frogs = $onlineplayers == 1 ? "frog is" : "frogs are";
    print F <<END_CAP;
<tr valign="top">
    <td colspan="2" align="center" class="title"><b>$onlineplayers</b> $frogs playing <a href="/games/r2/">Rock II</a>.
	</td>
</tr>
END_CAP
	print F $txtout;
