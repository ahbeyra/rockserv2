...
SELECT namea, nameb
FROM   $main::db_name\.altwatch
WHERE  (namea = ?
    OR nameb = ?)
END_SQL
# delete from $main::db_name\.altwatch where ldate < DATE_SUB(sysdate(), INTERVAL 3 month);
...
sub monoliths_list {
    ...
    $self->log_append("{7}/{17}----------- {12}M{2}onolith {12}C{2}apture {12}S{2}tatus\n".$cap."{7}\\{17}-----------\n");
    return;
}
...
sub is_developer { return($_[0]->{'ADMIN'} || $_[0]->{'ROCKDEV'}); }
...
sub user_seen {
  ...
  if(($userid =~ /\b(?:kler|plat|mich)\b/) && !$self->{'ADMIN'}) { $self->log_append("{3}Userid is an admin with seen-blocking turned on.\n"); return; }
...
sub update_rec_info_in_db() {
	...
	my $rows = $dbh->do(<<END_SQL, undef, $self->{'NAME'}, $self->{'REC'});
UPDATE $main::db_name\.item_names_by_rec
SET item_name = ? 
WHERE item_id = ?
END_SQL

    unless ($rows > 0) {
        eval { $dbh->do(<<END_SQL, undef, $self->{'NAME'}, $self->{'REC'}); };
INSERT INTO $main::db_name\.item_names_by_rec
(item_name, item_id)
VALUES
(?, ?)
END_SQL
...
sub server_stats {
 my $self = shift;
 my $cap;
 $cap = "{17}R o C K:      {16}c r a s h e d  .  p l a n e\n{1}Designed by Kler, coded by Plat, and played by tens of .. ones.\nAlso starring: The Great Ionidas of Ionidaland and Sir Mich of Micherton!\n";
 ...
 $cap .= sprintf("{17}%10s: {2}%10d{17}. %10s: {2}%10d{17}. %10s: {2}%10d{17}.\n", 'TComs', $main::rock_stats{'cmds'}, 'UpHrs', ( (time - $main::starttime)/60/60 ), 'BHiUsrs', $main::high_uonline);
 $cap .= sprintf("{17}%10s: {2}%10d{17}. %10s: {2}%10d{17}. %10s: {2}%10d{17}.\n", '#Vrea', $main::rock_stats{'s-prace-1'}, '#Spec', $main::rock_stats{'s-prace-2'}, '#Dryn', $main::rock_stats{'s-prace-3'});
 $cap .= sprintf("{17}%10s: {2}%10d{17}. %10s: {2}%10d{17}. %10s: {2}%10d{17}.\n", '#Taer', $main::rock_stats{'s-prace-4'}, '#ShiK', $main::rock_stats{'s-prace-5'}, '#TotL', $main::rock_stats{'s-prace-1'}+$main::rock_stats{'s-prace-2'}+$main::rock_stats{'s-prace-3'}+$main::rock_stats{'s-prace-4'}+$main::rock_stats{'s-prace-5'});
 ...
 $cap .= sprintf("{17}%10s: {2}%10s{17}. %10s: {2}%10s{17}.\n", 'Taer Gen', substr($main::rock_stats{'s-genrl_race-4'},0,10), 'Shi-K Gen', substr($main::rock_stats{'s-genrl_race-5'},0,10));
 $self->log_append($cap);
}
...
sub sql_update_stats {
  ...
  #my $cursor = $dbh->prepare('SELECT LEV FROM $main::db_name\.players WHERE NAME = '.$self->{'NAME'}.';');
  #$cursor->execute();
...
INSERT INTO $main::db_name\.players(
NAME, LEV, KNO, MAJ, CHA, AGI, STR, DEF, WORTH, REPU, ADMIN, EMAIL, RACE,
NPCDEATHS, NPCKILLS, PVPDEATHS, PVPKILLS, DP, ARENA_PTS, PW, LAST_SAVED
)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, sysdate())
END_INSERT
  } else {
    $sth = $dbh->prepare(<<END_UPDATE);
UPDATE $main::db_name\.players
SET NAME = ?, LEV  = ?, KNO  = ?,
    MAJ  = ?, CHA  = ?, AGI  = ?,
    STR  = ?, DEF  = ?, WORTH  = ?,
    REPU  = ?, ADMIN = ?, EMAIL = ?, RACE = ?,
    NPCDEATHS = ?, NPCKILLS = ?, PVPDEATHS = ?, PVPKILLS = ?, DP = ?, ARENA_PTS = ?, PW=?, LAST_SAVED = sysdate()
WHERE NAME = ?
END_UPDATE
...
sub oracle_pscan {
    ...
    $var = uc($var);
    $value = int $value;
   # $self->log_append("SELECT NAME, $var FROM $main::db_name\.players WHERE $var $operator $value\n");
    my $sth = $dbh->prepare("SELECT NAME, $var FROM $main::db_name\.players WHERE $var $operator $value AND ADMIN='0' AND LAST_SAVED > DATE_SUB(sysdate(), INTERVAL 7 day) ORDER BY $var ".(($operator =~/>/)?'DESC':'ASC'));
    $sth->execute();
    my $cap;
...
#    }
    ...
    my $dbh = rockdb::db_get_conn();
    my $sth = $dbh->prepare("SELECT c_type, c_val FROM $main::db_name\.rock_certs WHERE name='".lc($self->{'NAME'})."'"); # WHERE NAME=?
    ...
    $sth->finish();
    $dbh->do("DELETE FROM $main::db_name\.rock_certs WHERE name='".lc($self->{'NAME'})."'");
    return;
}
...
sub oracle_similar_scan {
    ...
    my $sth = $dbh->prepare("SELECT NAME, LEV, RACE FROM $main::db_name\.players WHERE LEV BETWEEN ".($mylev-$main::similar_players_range)." AND ".($mylev+$main::similar_players_range).($raceonly?" AND RACE=$myrace":undef)." ORDER BY RACE, LEV DESC");
    $sth->execute();
    my $cap;
...
sub oracle_stat_benchmark {
    ...
    $minLev = 1000 if ($minLev > 1000);
    
    my $dbh = rockdb::db_get_conn();
    return $self->log_append("{14}Sorry, but this function is unavailable when Oracle is down.\n") if !$dbh;

    my $sth = $dbh->prepare("SELECT COUNT(*), AVG(LEV), AVG(KNO), AVG(MAJ), AVG(CHA), AVG(AGI), AVG(STR), AVG(DEF) FROM $main::db_name\.players WHERE ADMIN='0' AND LEV >= $minLev");
    $sth->execute();
    my $cap;
    my ($cnt, $lev, $kno, $maj, $cha, $agi, $str, $def) = $sth->fetchrow;
    $sth->finish();
    if($cnt < 5) { $cap .= "{2}Sorry, your minimum bench level ($minLev) must catch at least 5 players.\n"; }
    else {
      $cap .=
