...
sub ora_players_count {
    ...
    my $sth = $dbh->prepare("select count(*) as player_count, race from $main::db_name\.r2_players where LAST_SAVED > SUBDATE(Now(), INTERVAL 5 day) group by race");
    $sth->execute();
    my $total_count = 0;
...
sub ora_players_load_where {
    ...
    my $dbh = rockdb::db_get_conn();
    my $sth = $dbh->prepare("SELECT NAME, RACE FROM $main::db_name\.r2_players WHERE $where");
    $sth->execute();
...
### LOAD ABSOLUTE STATS FOR MORE FUNNESS ###
    my $dbh = rockdb::db_get_conn(); 
    my $sth = $dbh->prepare("SELECT * FROM $main::db_name\.r2_players WHERE lower(name)=?");
    $sth->execute($lc_name);
