...
my %action_log_commands = map { $_ => 1 } qw(
    a auction bank bid buy cast deposit emote follow give i leave raise remove
    say score shout skills st tell think wear who withdraw
);
my $action_log_ready;

sub action_log_ready {
    return $action_log_ready if defined $action_log_ready;
    $action_log_ready = 0;
    my $dbh = rockdb::db_get_conn();
    eval {
        $dbh->do(<<'END_SQL');
CREATE TABLE IF NOT EXISTS r2_action_log (
  entry_id int UNSIGNED NOT NULL AUTO_INCREMENT,
  uin int UNSIGNED NOT NULL,
  player_name varchar(30) NOT NULL DEFAULT '',
  command varchar(40) NOT NULL DEFAULT '',
  ...
  KEY idx_action_uin_date (uin, entry_date),
  KEY idx_action_command (command),
  KEY idx_action_player (player_name)
...
END_SQL
    ...
    return $action_log_ready == 1;
}

sub log_action_command {
    my ($player, $command, $args) = @_;
    return unless $player && $player->{'TYPE'} == 1;
    return unless $player->{'UIN'};
    $command = lc($command || '');
    return unless $action_log_commands{$command};
    $args = '' unless defined $args;
    $args =~ s/[^ -~]/?/g;
    $args = substr($args, 0, 200);
    &log_action($player, $command, $args);
    return;
}

sub log_action {
    my ($player, $command, $args) = @_;
    return unless action_log_ready();
    my $dbh = rockdb::db_get_conn();
    my $uin = int($player->{'UIN'} || 0);
    return unless $uin;
    my $name = $player->{'NAME'} || '';
    my $room_id = defined $player->{'ROOM'} ? int($player->{'ROOM'}) : undef;
    my $race = defined $player->{'RACE'} ? int($player->{'RACE'}) : undef;
    my $level = defined $player->{'LEV'} ? int($player->{'LEV'}) : undef;
    my $ip = $player->{'IP'} || '';
    eval {
        $dbh->do(<<'END_SQL', undef, $uin, $name, $command, $args, $room_id, $race, $level, $ip);
INSERT INTO r2_action_log
(uin, player_name, command, args, room_id, race, level, ip_addr, entry_date)
VALUES
(?, ?, ?, ?, ?, ?, ?, ?, sysdate())
END_SQL
    };
