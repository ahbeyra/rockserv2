    ...
    if($killer) {
        ...
        if( $self->{'TYPE'}==1 && $killer->{'TYPE'}==1 ){ 
            ...
            if ($killer->{'TYPE'}==OTYPE_NPC  && $self->{'TYPE'} != OTYPE_NPC) {
                $self->{'NPCDEATHS'}++;
                my $cap = "{14}Sorrow.. \{$self->{'RACE'}\}$self->{'NAME'} {7}was slain by \{$killer->{'RACE'}\}$killer->{'NAME'}\{7}!\n";
                &main::rock_rshout($self, $cap, 'silence deaths');
                $main::rock_talker->broadcast('R2SHOUT', $cap) if defined $main::rock_talker;
...
sub cmd_do {
  ...
  my $log_args = '';
  if (@cap > 1) {
      $log_args = join(' ', @cap[1..$#cap]);
  }
  &main::log_action_command($player, $cap[0], $log_args);
