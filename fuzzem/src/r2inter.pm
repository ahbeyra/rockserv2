...
sub rockserv_apply_env_prefs {
    my $autocleanup = $ENV{'ROCKSERV_AUTOCLEANUP'};
    return unless defined $autocleanup;
    if ($autocleanup =~ /^(0|off|false|no)$/i) {
        $main::do_cleanup = 0;
    } else {
        $main::do_cleanup = 1;
    }
...
sub rockserv_apply_restart_notice {
    my $enabled = $ENV{'ROCKSERV_RESTART_NOTICE'};
    return if defined $enabled && $enabled =~ /^(0|off|false|no)$/i;
    my $notice_file = $ENV{'ROCKSERV_RESTART_NOTICE_FILE'} || "$main::base_code_dir/restart_notice.txt";
    return unless $notice_file && -e $notice_file;
    open(my $fh, '<', $notice_file) or return;
    local $/;
    my $notice = <$fh>;
    close($fh);
    ...
    $notice =~ s/\s+\z//;
    return if $notice eq '';
    if (defined $main::motd && $main::motd ne '') {
        $main::motd .= "\n" . $notice . "\n";
    } else {
        $main::motd = $notice . "\n";
    }
...
sub rockserv_heartbeat {
    my $path = $ENV{'ROCKSERV_HEARTBEAT_FILE'} || "$main::base_code_dir/heartbeat.txt";
    if ($path && open(my $fh, '>', $path)) {
        print $fh time;
        close($fh);
    }
    my $interval = $ENV{'ROCKSERV_HEARTBEAT_INTERVAL'} || 30;
    $interval = 30 if $interval !~ /^\d+$/ || $interval < 10;
    $main::eventman->enqueue($interval, \&main::rockserv_heartbeat);
    return;
}
