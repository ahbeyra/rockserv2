...
sub user_support {
	...
	if( ($self->{'TYPE'}!=1) || ($self->{'RACE'}<1) || ($self->{'RACE'}>6) ) { $self->log_error("You cannot vote until you are one of the pure races."); return; } 
...
#		    $self->log_error("Admins have more to worry about than the quest for the eternal power of their race.");
			...
			$main::general_votes{lc($self->{'NAME'})} = pack('LA*', $targ->{'RACE'}, $uid, $targ->{'REPU'});
    	    $self->log_append("{2}You throw your support behind {12}$targ->{'NAME'}\'s{2} campaign for power.\n");
    	    $self->{'VOTING4'} = $targ->{'NAME'};
	    }
		return;
	}
}
...
sub on_hdigest {
	...
	if (ref($self) eq 'bodypart') {
		...
		}else{	    $eater->log_error("You sick, sick person.");	    }
	} elsif (!$self->{'DIGEST'}) { 
...
sub salp {
	...
	if( ($salper->{'RACE'} != 4) && !$salper->skill_has(13) ){
		...
		my $dam = int ( ($salper->{'SALP'}*2.2 + ($salper->{'SALP'}*2.5))
		                * rand(1.5)
						* ($salper->{'LEV'}/25 + 1)
					  );
    	
		if($dam > ($salper->{'LEV'}*8.6)) {
		    $dam = int ($salper->{'LEV'}*8.6 + rand(5));
		}
		if($victim->aprl_rec_scan(719))
		{
			my $olddam = int ($dam);
			
			$dam = int rand($dam*(1/2));
			my $shielded = $olddam - $dam;
			$victim->log_append("{15}Your Scaled Cloak shields you from $shielded damage.\n");
		}
	...
	if(my $recip = $self->uid_resolve($uid)) {
    	...
    	if (defined($recip->{'FX'}->{'25'}) && !$recip->{'ADMIN'}) {
    		...
    		foreach $player (sort keys(%{$main::activeuids})) {
        		...
        		next if (!$player->{'ADMININVADE'});
				$player->log_append('{16}'.($self->{'NICK'} || $self->{'NAME'})." {14}tells {16}$recip->{'NAME'} {14}[privately]: {17}$message\n");
        		$n++;
    }
...
sub uid_ignore {
	...
	if (!$uid) { 
    	...
    	if($recip->{'ADMIN'}){$self->log_append("{3}You may not ignore this person.\n"); return;}
...
sub canLogoutOnTelDiscon {
	...
	if ($main::map->[$self->{'ROOM'}]->{'HANGUP_DEATH'}) { 
    	...
    	return 0;
    		...
    		$self->log_append("{14}** Cannot log off while in an arena). **\n")
	        unless $quiet;
    	return 0;
	}else { return 1; }
}
...
sub old_who {
    ...
    foreach $player (sort keys(%{$main::activeuids})) {
        ...
        $cap .=  sprintf ("\{%d\} \%16s $wingColor\%s%1s$wingColor %s",
            ...
            $main::desertmap[$player->{'PACIFIST'}] || $main::generalmap[$player->{'GENERAL'}] || $main::asteriskmap[$player->{'SOLDIER'} !=0] || 'o',
...
sub who {
    ...
    foreach $player (sort keys(%{$main::activeuids})) {
        ...
        next if (time - $player->{'@LCTI'}) > 60;
        my $wingColor1 = $main::wingmap1[int ($player->{'LEV'}/25)] || '{11}!';
        my $wingColor2 = $main::wingmap2[int ($player->{'LEV'}/25)] || '{11}!';
	my $top_player = top_player();

		#if($player->{'NAME'} eq $top_player){ $player->{'TOP_PLAYER'} = 1; }
		#$main::topplayermap[$player->{'TOP_PLAYER'}]
        $cap .=  sprintf ("\{%d\} \%16s $wingColor1\%s%1s$wingColor2 %s",	
            ($player->{'RACE'} % 18),
            $player->{'NAME'},
            $main::gendermap{$player->{'GENDER'}},
            $main::desertmap[$player->{'PACIFIST'}] || $main::generalmap[$player->{'GENERAL'}] || $main::asteriskmap[$player->{'SOLDIER'} !=0] || 'o',
            $main::pkmap[$player->{'PKTIME'} && ((time-$player->{'PVPTIME'})<60*2)] || $main::badrepmap[$player->{'REPU'}<$main::badrepborder] || $main::goodrepmap[$player->{'REPU'}>$main::goodrepborder] || $main::afkmap[$player->{'AFK'}] || $main::adminmap[$player->{'ADMIN'}] || $main::developermap[$player->{'ROCKDEV'}] || $main::newbiemap[$player->{'NEWBIE'}!=0] ||  '     ');

         if ($n) { $cap .= "\n"; } else { $cap .= ' '; }
         $n = abs($n-1);
		 $player->{'TOP_PLAYER'} = 0;
    }
    if($n) { $cap .= " {1}oO {6}Rock: Reloaded{1} Oo\n"; }
    $cap .= "{41}{14}There are {5}$playercount {14}total players in the game right now.\n";
    $self->log_append("{40}{13}Users active within the last minute:\n".$cap.'{41}');
    return;
}
...
sub who_admin {
    ...
    foreach $player (sort keys(%{$main::activeuids})) {
        ...
        $cap .=  sprintf ("{%d\}%20s %20s %20s\n", ((time - $player->{'@LCTI'}) > 60 ?7:17), $player->{'NAME'}, $main::adminmap[$player->{'ADMIN'}] || $main::developermap[$player->{'ROCKDEV'}!=0] || '{7}Staff', $main::caarpmap[$player->{'OVERSEER'}!=0]);
        $n++;
    }
...
sub who2 {
 ...
 foreach $player (sort keys(%{$main::activeuids})) {
    $player = &main::obj_lookup($main::activeuids->{$player});
	  ...
	  my $top_player = top_player();
	  if($player->{'NAME'} eq $top_player){ $player->{'TOP_PLAYER'} = 1; }
      $cap = "$main::desertmap[$player->{'PACIFIST'} != 0]$main::generalmap[$player->{'GENERAL'} != 0]$main::asteriskmap[$player->{'SOLDIER'} != 0]$main::topplayermap[$player->{'TOP_PLAYER'}]$main::adminmap[$player->{'ADMIN'}]$main::caarpmap[$self->{'OVERSEER'}]$main::developermap[$player->{'ROCKDEV'}]$main::newbiemap[$player->{'NEWBIE'}!=0]$main::afkmap[$player->{'AFK'}]";
...
sub rest
{
	my $self = shift;
	my ($dam);
	my $monolith_own = $self->race_owns_monolith('monolith_granite');
	my $rest_turns = 3;
	if($monolith_own){ $rest_turns = 1;}
	...
	if(!$self->can_do(0,0,$rest_turns+4*$self->is_tired()))
    {
        return 0;
    }
...
sub exp_need_ast {
  ...
  my $auto_prefix = (!$_[0]->{'AUTORAISE_STATS'} || $_[0]->{'AUTORAISE_STATS'}->{$statnum}) ? '{4}A' : ' ';
  	if($_[0]->{'EXP'}->[$statnum] >= 8000000000) { return $auto_prefix . ' '; }
...
sub peek(dir) {
	...
	if((!$farsee && $room->{'NOPEEK'}) && ($self->{'RACE'}!=6)) { $self->log_append('{4}'.$room->{'NOPEEK'}."\n"); return; }
	
	my $turns_taken = $self->aprl_rec_scan(585)?0:2; # no peeky turns if they have magic goggles
 	if( $self->race_owns_monolith('monolith_optical') && $self->{'TYPE'}==1){
	 $turns_taken = 0;
     }
	...
	if($farsee) {
		...
		$self->log_append("{17}Players were not notified of your vision skills.\n");
 	}else {
...
sub thanked {
	...
	$p->log_suspicious_activity("Tried to thank $self->{'NAME'}");

    if (!$p->{'ADMIN'}) {
        if($p->{'LEV'}<45) { $p->log_error("You must be at least level 45 to thank a player."); return 1; }
        if(!($main::allyfriend[$self->{'RACE'}]->[$p->{'RACE'}])) { $p->log_error("You can only thank characters of your race or an allied race."); return 1; }
...
sub deprecated {
    ...
    $p->log_suspicious_activity("Tried to deprecated $self->{'NAME'}");

    if (!$p->{'ADMIN'}) {
        if($p->{'LEV'}<45) { $p->log_error("You must be at least level 45 to deprecate a player."); return 1; }
        if(!($main::allyfriend[$self->{'RACE'}]->[$p->{'RACE'}])) { $p->log_error("You can only deprecate characters of your race or an allied race."); return 1; }
...
sub luck_status {
	if($_[0]->{'LUCK'}>99){
		...
		return "{12}the {13}Perfect Luck {12}of the {13}Irish";
	}
  return ($_[0]->{'LUCK'}<=0)?$main::luckstatuslist[0]:$main::luckstatuslist[int (10*$_[0]->{'LUCK'}/100)];
}
...
sub inv_remnodb {
   ...
   my $self = shift;
   my $i;
   if (!$self->{'DB'} || !(@{$main::db->[$self->{'DB'}]})) { foreach $i ($self->inv_objs) { $i->dissolve_allsubs; } } # recursive dissolve
   else {
      my (%db);
...
sub course_add {
  ...
  foreach my $course (@_) {
    ...
    elsif($main::map->[$self->{'ROOM'}]->{'ACADEMY'} ne $main::courses{$course}->[8]) { 
	#$self->log_append($main::map->[$self->{'ROOM'}]->{'ACADEMY'}); 
	#$self->log_append($main::courses{$course}->[8]); 
	$self->log_append("{17}This isn't the right place to sign up!\n"); 
	return; }
...
sub course_update (coursename){
  ...
  my $c = int ((10+7*$self->fuzz_pct_skill(0, 200)) / $keycount * $main::lightning_course_point_multiplier + ($self->{'LEV'}/100));  
...
sub daily_win {
	my ($self) = @_;
	$self->log_append("Check Out the Web Site to get your daily prize\n");
	return;
	...
	$self->do_daily_win();
	$self->room_tell("{13}$self->{'NAME'} won a daily prize!\n");
}
...
sub clean_daily_win()
{
	my $self = shift;
	$self->{'DAILYWIN'} = 2;
	$self->log_append("{11}Daily Win Cleared\n");
}

sub do_daily_win() 
{
  my $self = shift;
  my $dailyprize = $main::daily_win_items[int rand(scalar @main::daily_win_items)];
  my $prizerec = $self->item_spawn($dailyprize);
  $self->log_append("{14}You won a $prizerec->{'NAME'}.\n");
  if($prizerec->{'REC'} == 301){
	  $self->cmd_do("use tarnished");
  } if($prizerec->{'REC'} == 540){
	  $self->cmd_do("use super-duper");
  }if($prizerec->{'REC'} == 287){
	  $self->cmd_do("use planar clock");
  }
  $self->{'DAILYWIN'} = 1;
}
...
sub pstat_raise (stat, amt) {
	...
	if( ($stat>=6 && $stat<=22) || defined($main::parr{$stat})) {
    	 ...
    	 if($self->{'EXP'}->[$stat] >= 8000000000) {
             $self->log_append("{1}You cannot raise your $sname any further.\n");
             #$self->cmd_do("eraise $sname $amt");
             return(0);
    	 }
    	 
		 my $rby=0;
		 if($self->{'LEV'} >= 1000  && $self->skill_has(63))
		 {
    	 for (my $n=1; $n<=$amt; $n++) { 
        	 last if($self->{'EXP'}->[$stat] >= 8000000000);
...
sub prayed{ $_[1]->log_append("{3}..you remember all the pointless pain at stake and quickly change your mind.\n"); }
...
sub skills_list {
  ...
  if($main::rock_stats{'monolith_vindicator'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}S{16}wi{17}mm{16}in{6}g','', '(implicit swimming)')); }
  if($main::rock_stats{'monolith_optical'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}E{16}as{17}y Peek{16}in{6}g','', '(implicit turn free peeking')); }
  if($main::rock_stats{'monolith_granite'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}E{16}as{17}y Rest{16}in{6}g','', '(implicit one turn resting)')); }
  if($main::rock_stats{'monolith_advocate'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%11s \{6}\%s\n", '{6}I{16}nc{17}reased Invent{16}or{6}y','', '(implicit maxinv +2)')); }
  if($main::rock_stats{'monolith_protector'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}K{16}ni{17}tFl{16}es{6}h','', '"knit flesh"')); }
  if($self->{'SOLDIER'} && $main::rock_stats{'monolith_vindicator'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}Dee{16}p B{17}rea{16}th{6}ing','', '(implicit diving)')); }
  ...
  if($self->{'SOLDIER'} && $main::rock_stats{'monolith_optical'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}W{16}iz{17}ards{16}Ey{6}e','', '"wizeye <victim>"')); }
  if($self->{'SOLDIER'} && $main::rock_stats{'monolith_granite'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%11s \{6}\%s\n", '{6}L{16}es{17}ser Rejuvenat{16}io{6}n','', '"lrejuv"')); }
  if($self->{'SOLDIER'} && $main::rock_stats{'monolith_advocate'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%11s \{6}\%s\n", '{6}I{16}nc{17}reased Invent{16}or{6}y (S)','', '(implicit maxinv +3)')); }
  if($self->{'SOLDIER'} && $main::rock_stats{'monolith_protector'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}C{16}lo{17}seWou{16}nd{6}s','', '"close wounds"')); }
  ...
  if($self->{'GENERAL'} && $main::rock_stats{'monolith_optical'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}A{16}r{17}eaM{16}a{6}p','', '"amap"')); }
  if($self->{'GENERAL'} && $main::rock_stats{'monolith_granite'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%11s \{6}\%s\n", '{6}G{16}re{17}ater Rejuvenat{16}io{6}n','', '"grejuv"')); }
  if($self->{'GENERAL'} && $main::rock_stats{'monolith_advocate'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%11s \{6}\%s\n", '{6}I{16}nc{17}reased Invent{16}or{6}y (G)','', '(implicit maxinv +4)')); }
  if($self->{'GENERAL'} && $main::rock_stats{'monolith_protector'}==$self->{'RACE'}) { push(@cap, sprintf("\{16}\%-30s%18s \{6}\%s\n", '{6}S{16}pe{17}edH{16}ea{6}l','', '"speed heal"')); }
...
sub race_ally() {
  ...
  if(($self->{'RACE'}<1) || ($self->{'RACE'}>6)) { $self->log_append("{17}..not while you're $main::races[$self->{'RACE'}].\n"); return; }
  if(($ally<1) || ($ally>6)) { $self->log_append("{17}Even YOU cannot ally with that race!\n"); return; }
  ...
  if($main::rock_stats{'rally-'.$self->{'RACE'}.'-'.$ally} ^= 1) { 
     ...
     if($main::rock_stats{'rally-'.$ally.'-'.$self->{'RACE'}}) {
          &main::rock_shout(undef, "{17}***** The {16}ALLIANCE {17}between the {2}$main::races[$self->{'RACE'}] {17}and{2} $main::races[$ally] {17}retracted by the $main::races[$self->{'RACE'}] in 3 hours.\n");
          # delete the other race's offer, if it was broken off..
		  
		  $main::eventman->enqueue(60*60*3, sub {
				delete $main::rock_stats{'rally-'.$ally.'-'.$self->{'RACE'}};
				$main::allyfriend[$self->{'RACE'}]->[$ally] = $main::allyfriend[$ally]->[$self->{'RACE'}] = undef;
				&main::rock_shout(undef, "{17}***** The {16}ALLIANCE {17}between the {2}$main::races[$self->{'RACE'}] {17}and{2} $main::races[$ally] {17}retracted by the $main::races[$self->{'RACE'}].\n");
								 });
...
sub msg_broadcast {
    my ($self, $msg) = @_;
    	    #$self->pref_toggle('censor filter', 1, 1);
    ...
    if(!$self->can_do(0,0,0)) { return; } # was 2 turns 2002-12-29
    $msg = &main::text_filter_game($msg, $self);
	  # Get an array of color codes.
		if($self->{'RAINBOW'}){
			my @chars; @chars = split(/ /,$msg); foreach(@chars){
			my $lastrand = 0;
			my $rand = 0;
			$rand = int(rand((16 - 10) + 1)) + 11;
			
			if (($rand > 6)&&($rand<11)){$rand = 11; }
			$_ = '{'.$rand.'}'.$_;	}$msg =join(' ', @chars);
		}
		
	
    my $cmsg = censored_message->new_prefiltered("{1}.o{17}\_/{7}-{17}($self->{'NAME'})\{7}: {".($self->{'PENCOLOR'}||16)."}$msg\n", $self);
...
sub pen_color {
    my ($self, $color) = @_;
    $color = lc($color);
	if($color eq 'multi'){$self->{'RAINBOW'}=1; $self->log_append("{13}Switched pen color to multicolor.\n");return;}else{$self->{'RAINBOW'}=0;}
    ...
    else { $self->log_append("{17}Syntax: pen <color|1-7|11-17|multi>\n"); }
    return;
}
...
sub dp_calc {
  ...
  my $dp = $self->{'DP'}; # 1 point per day general, per monolith
                          ...
                          # 5 points for striking blow to monolith guardians
...
SELECT A.item_desc as item_desc, USELL.member_name as seller_uid, A.start_date as start_date, A.end_date as end_date, A.item_name as item_name, A.item_level as item_level
FROM $main::db_name\.r2_members USELL, $main::db_name\.r2_auctions A
WHERE
     A.auction_id = ? AND
	 USELL.id_member = A.seller_uin
END_SQL
...
SELECT A.auction_id, A.item_name, A.min_price, A.high_bid, USELL.member_name, UBUY.member_name, A.bid_increment, A.end_date
FROM $main::db_name\.r2_members USELL,  $main::db_name\.r2_members UBUY RIGHT OUTER JOIN $main::db_name\.r2_auctions A on A.high_bid_uin = UBUY.id_member
WHERE
...
END_SQL
    ...
    foreach my $row (@$rows) {
		...
		# same table :(
		my ($my_max_bid) = rockdb::sql_select_row("SELECT max_bid FROM $main::db_name\.r2_auction_bids WHERE auction_id=? AND bidder_uin=?", $row->[0], $self->{'UIN'});
...
SELECT A.auction_id, 
	...
	UBUY.member_name, 
	A.end_date
FROM $main::db_name\.r2_members UBUY INNER JOIN $main::db_name\.r2_auctions A ON A.high_bid_uin = UBUY.id_member, 
	$main::db_name\.r2_members USELL
WHERE sysdate() > A.end_date AND USELL.id_member = A.seller_uin
...
END_SQL
    ...
    foreach my $row (@$rows) {
	    ...
	    # itemid, item name, seller, bid/min, bidder
	    $txt .= sprintf "{7}%6d] {17}%15s {13}%7s {3}%10s\n", $row->[0], substr($row->[1],0,15), &commify($row->[3]?$row->[3]+$row->[6]:$row->[2]||$row->[6]), substr($row->[5], 0, 10)||"<none>", substr($row->[7], 5), ($my_max_bid?&commify($my_max_bid):'---');
	}
	
	if ($txt) {
	    $txt  = sprintf ("{7}%6s] {17}%15s {13}%7s {3}%10s\n", "#", "Item Name",  "Min Bid", "Top Bidder") . $txt;
	    $self->log_append("{17}\nThe following auctions have ended recently:\n".$txt);
	} else {
	    $self->log_append("{3}<<  No items are currently being auctioned. Try again later.  >>\n");
	}
...
##	if ($self->{'LEV'} < 100) { $self->log_append("{3}<<  You must be at least level 100 to auction stuff (for now - just testing with a smaller group of people).  >>\n"); return 0; }
	...
	if ($min_price > 70_000) { $self->log_append("{3}<<  Minimum price must be less than 70,000 cryl.  >>\n"); return 0; }
...
#    $self->room_sighttell("{7}$self->{'NAME'} {3}puts $item->{'NAME'} up for auction.\n");
    ...
    my $item_level = $item->{'MINLEV'};
    if(!$item_level){$item_level=0;}
	my $dbh = rockdb::db_get_conn();
	$dbh->do(<<END_CAP, undef, $auction_minutes, $min_price, $bid_increment, $self->{'UIN'}, &stripColorCodes($item->{'NAME'}), &stripColorCodes($item->desc_get()), $item_data, $item_level);
INSERT INTO $main::db_name\.r2_auctions
(start_date, end_date, min_price, bid_increment, seller_uin, item_name, item_desc, item_data, item_level)
VALUES
...
FROM $main::db_name\.r2_auctions A
WHERE
      A.auction_id = ?          AND
	  sysdate() < A.end_date
ORDER BY A.end_date ASC
END_SQL
     ...
     my ($cryl_prebidded) = rockdb::sql_select_row("SELECT max_bid FROM $main::db_name\.r2_auction_bids WHERE bidder_uin=? AND auction_id=?", $self->{'UIN'}, $auction_id);
...
UPDATE $main::db_name\.r2_auction_bids
...
END_SQL
	     ...
	     # insert
		 $dbh->do(<<END_SQL, undef, $bid, $auction_id, $self->{'UIN'});
INSERT INTO $main::db_name\.r2_auction_bids
...
END_SQL
	 ...
	 if ($row->{'HIGH_BID_UIN'} == $self->{'UIN'}) {
		 ...
		 my ($public_bid, $public_bidder_uin) = ($min_bid, $self->{'UIN'});
		 my $outbidders = rockdb::sql_select_mult_row("SELECT B.max_bid, B.bidder_uin, A.member_name FROM $main::db_name\.r2_auction_bids B, $main::db_name\.r2_members A WHERE B.auction_id=? AND B.bidder_uin = A.id_member AND B.max_bid >= ? ORDER BY max_bid DESC, bid_date DESC", $auction_id, $min_bid + $row->{'BID_INCREMENT'});
...
sub auction_claim {
	...
	if(!$self->inv_free()){$self->log_append("Free up some inventory room first, then we will see if there is anything for you to claim.\n"); return;}
	
	my $rows = rockdb::sql_select_mult_row(<<END_SQL, $self->{'UIN'}, $self->{'UIN'});
SELECT auction_id, item_data, high_bid_uin
FROM $main::db_name\.r2_auctions
