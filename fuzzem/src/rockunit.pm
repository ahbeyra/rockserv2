...
sub on_idle {
  ...
  if( ($self->{'RACE'}==4) ) {
    ...
    $self->log_append("{3}Your stomach acids have further strengthened the salp inside it.\n")
        if $self->{'SALP'};
    $self->{'SALP'} = 99_999_999 if $self->{'SALP'} > 99_999_999;
  }
  $self->auto_loot();
  return;
}
...
#http://www.dillfrog.com/games/r2/help/exp_bonus.asp?new_exp_power=1.7&new_exp_multiplier=20&new_exp_add=0
  ...
  if($self->{'TYPE'}==1 && $victim->{'TYPE'}==2) {
     if(defined($self->{'A_HIST'}->{$victim->{'OBJID'}})) {
      ...
      #$self->{'KILLS'}->[$victim->{'REC'}] = 1;
		
        if($killPct >= .9 && $victim->{'REC'} && !vec($self->{'KILLREC'}, $victim->{'REC'}, 1)) {
#		 if($killPct >= .9 && $victim->{'REC'} && !$self->{'KILLS'}->[$victim->{'REC'}]) {
           if($victim->{'LIMIT'}==1) { 
             $ephy *= 4;
           } else {
...
sub desc_get {
    ...
    if($main::races[$self->{'RACE'}]) {
        $cap .= "{15}( {1}".$main::races[$self->{'RACE'}].'{7} of {1}' . ($self->{'GUILD'} || 'no guild')."{1} )\n";
    }
    if(defined($self->{'FX'}->{'33'})) {
        $cap .= "{2}$self->{'NAME'}\'s planar frequency must be garbled, for you cannot clearly see $self->{'PPRO'}.";
    } elsif($self->{'ADMIN'} && ($self->{'CANLOOK'}!=1) ) {
        $cap .= "{2}#### $self->{'NAME'} is one of those $main::adminmap[$self->{'ADMIN'}] {2}guys for the game. ####\n";
    } elsif(defined($self->{'FX'}->{'39'}) || ($self->{'SOLDIER'} && $main::rock_stats{'monolith_shadow'}==$self->{'RACE'}) && !$looker->{'ADMIN'} && $looker!=$self) {
        ...
        $cap .= "{4}$self->{'NAME'} {14}appears to be ".$self->health_status()."{7}.\n{2}$self->{'NAME'} {12}has ".$self->luck_status().". {4}.\n";
        my @objs = $self->inv_objs();
        my ($wearing,@carr);
        if(@objs) {
            my ($o);
            foreach $o (@objs) { 
                unless ($o->is_invis() && ($looker->{'RACE'}!=6)) {
	
				my $color = 17;
				my $rare_type ='(c)';
				
				if(($o->{'DRPPCT'}<=80)&&(defined($o->{'DRPPCT'}))){$color =17;$rare_type = '(c)';}
				if(($o->{'DRPPCT'}<=30)&&(defined($o->{'DRPPCT'}))){$color =16;$rare_type = '(u)';}
				if(($o->{'DRPPCT'}<=1)&&(defined($o->{'DRPPCT'}))){$color =14;$rare_type = '(r)';}
				if(($o->{'DRPPCT'}<=.5)&&(defined($o->{'DRPPCT'}))){$color =11;$rare_type = '(l)';}
				if(($o->{'DRPPCT'}<=.1)&&(defined($o->{'DRPPCT'}))){$color =13;$rare_type = '(m)';}
				if(defined($o->{'UPD'})){$color =14;$rare_type = '(r)';}
				
				#if(defined($o->{'WC'} < 65)){$color =17;$rare_type = '(c)';}
				if(($o->{'WC'}>=65)){$color =14;$rare_type = '(r)';}
				if(($o->{'WC'}>=80)){$color =11;$rare_type = '(l)';}
				if(($o->{'WC'}>=90)){$color =13;$rare_type = '(m)';}
				if(defined($o->{'LEGENDARY'})){$color =11;$rare_type = '(m)';}
				if($o->{'UNIQUE'}){$color =15;$rare_type = '(a)';}
				...
				my $carried = " {17}";
                    ...
                    if($o->{'WORN'}) {			
                        $wearing .= sprintf('{2}[ {12}%20s{2} ] {17}%s'."\n", $o->{'ATYPE'}, $color.$rare_type.$worn.$o->{'NAME'});
                    } elsif($self->{'WEAPON'} == $o->{'OBJID'}) {
                        $wearing .= sprintf('{6}[ {1}%20s{1} {6}] {11}%s'."\n", 'wielded', $color.$rare_type.$weapon.$main::objs->{$self->{'WEAPON'}}->{'NAME'});
                    } else {
                        push(@carr, $color.$rare_type.$carried.$o->{'NAME'});
                    }
                }
            }
        }
...
sub on_room_enter (objects: enterer; string: fromdir, observation) {
  my ($self, $who, $dir) = @_;
  #  || ($who eq $self) 
  # IF NOT ON OR NPC OR TIRED OR SAFE RETURN
...
sub on_ask {
 ...
 elsif ($topic =~ /stomach|food|hungry|hunger|eat|pear/) { $self->say('.... well I guess you could say I\'m on a pear diet - I know that sounds silly, but those are the doctor\'s orders... and the world outside is much too scary for a lady like me to leave this room.');  }
...
#  $self->say("Hey, thanks for the $obj->{'NAME'}, $from->{'NAME'}!");
  if(ref($obj) eq 'frog') { 
    if(ref($self) eq 'frog') {
      ...
      $self->{'CRYL'}+=8;
      $self->cmd_do('give 8 to '.$from->{'NAME'});
    }
  } elsif ($obj->{'NOSAVE'}) {
	$obj->dissolve_allsubs();
  }
...
sub on_room_exit (objects: exitee; string: fromdir, observation) {
  ...
  if ( $self ne $who ) { 
     if( $who->{'OBJID'} eq $self->{'STALKING'} ) { 
        $self->log_append('{3}Following '.$who->{'NAME'}."...(Typing {17}$dir {3}for you)\n");
        $self->cmd_do($dir);
     } elsif(!$who->{'ADMIN'} && ($self->{'LEV'} > 30) && !$self->cant_aggress_against($who, 1) && (!$who->{'SCURRYACTIVE'} || (rand(10)<2)) && (($who->{'HP'}/$who->{'MAXH'})<.25) && !$self->{'SENT'} && (rand(10)<7) && $main::dirlongmap{$dir} ) {
        $self->realm_move($dir);
        delete $self->{'TRD'};
...
sub on_room_enter {
   my ($self, $who, $dir) = @_;
    ...
    $main::events{$self->{'OBJID'}} ||= 5;
    	...
    	#if( $user->{'RACE'} == $self->{'RACE'}){
	   # 	$race_total++;
	   # 	#$self->room_sighttell("testing $race_total\n");
	   # 	}
    #}
    
    	if( $main::rock_stats{'armageddon_is_possible'} > 0){# && $race_total)	{  
	    	$self->{'IMMORTAL'} = 1; 
	    	$who->log_append("$self->{'NAME'} has become immortal.\n");
		} else { $self->{'IMMORTAL'} = 0; }
...
sub on_die {
  ...
  my $cap = "{3}*{13}##{16}o{13}##{3}*  {17}The {12}$main::races[$by->{'RACE'}]s {17}have captured {2}$main::monolithtoname{$self->{'MONOLITH'}}\{17}!\n         {17}[ {7}Type {17}monoliths {7}for the breakdown {17}]\n";
  &main::rock_shout(undef, $cap);
  $main::rock_talker->broadcast('R2SHOUT', $cap) if defined $main::rock_talker;
  ...
  my $capture_time = "$self->{'MONOLITH'}_time";
	$main::rock_stats{$capture_time} = time;
	
	my $capture_by = "$self->{'MONOLITH'}_killedby";
	$main::rock_stats{$capture_by} = $by->{'NAME'};
...
UPDATE
$main::db_name\.r2_monolith_capture_status
SET owned_by_race = ?,
    captured_by_uin = ?,
...
$prize_count++ if (($hours_alive) < 6 && (rand(100)<30));
  $prize_count++ if $hours_alive >= 6;
...
package npc_hallucination_monolith_guardian;
@npc_hallucination_monolith_guardian::ISA = qw ( monolith_guardian );
use strict;
use const_stats;

sub attack_sing {
    my ($self, $targ, $weapon) = @_;
	if ($targ->{'TYPE'} == 1 && rand(100) < 70) {
		$self->attack_melee($targ, $weapon); 
		if(!$targ->aprl_rec_scan(1)){
			if(my $spore = $main::map->[$self->{'ROOM'}]->item_spawn(1303)) {
				$spore->room_tell("{4}Hallucinator distorts!\n");
				$spore->attack_player($targ, 1);
				$spore->{'RACE'} = $main::rock_stats{'monolith_hallucination'};
	...
	return $self->SUPER::attack_sing($targ, $weapon);
}

sub c_crystal_spawn {
    my $spore = shift;
    $spore->room_sighttell("{4}$spore->{'NAME'} splits in two!\n");
    my $spawn = $main::map->[$spore->{'ROOM'}]->item_spawn(1304);
    $spore->dissolve_allsubs();
    $spawn->{'RACE'} = $main::rock_stats{'monolith_hallucination'};
    my $destruct_time = 1;
    $spawn->delay_room_sighttell($destruct_time, "{4}$spawn->{'NAME'} vanishes.\n");
...
sub on_idle {
 my $self = shift;
 if(my $spore = $main::map->[$self->{'ROOM'}]->item_spawn(1303)) {
			$spore->room_tell("{4}Hallucinator splits in two!\n");
			$spore->{'RACE'} = $main::rock_stats{'monolith_hallucination'};
...
package npc_vitalizer_monolith_guardian;
@npc_vitalizer_monolith_guardian::ISA = qw( monolith_guardian );
use strict;
use const_stats;
#(22:59:38) Kler: The Time Watcher hits you with his mighty time sucking attack, draining 400 turns!
#(23:02:44) Kler: I really think we should make that... maybe like .5% chance per hit..
...
sub attack_sing {
    ...
    # possibly throw a spell on 'em
    if ($victim->{'TYPE'} == 1 && rand(100) < 30) {
        my $vigor_lost = int(rand(2));
        $self->room_sighttell("{11}$self->{'NAME'} {1}hits {11}$victim->{'NAME'} {1}with $self->{'PPOS'} mighty time-sucking attack, draining their vigor!\n", $victim);
        $victim->log_append("{11}$self->{'NAME'} {1}hits you {1}with $self->{'PPOS'} mighty energy attack, draining you of your vigor!\n");
        $victim->{'VIGOR'} -= $vigor_lost;
        $self->note_attack_against($victim);
        $self->make_tired();  # Important: if you don't do this, you'll end up in a recursive loop
    }
...
package npc_crimson_monolith_guardian;
@npc_crimson_monolith_guardian::ISA = qw( monolith_guardian );
use strict;
use const_stats;
...
sub attack_sing {
	...
	$self->spell_hgeneric('BBATH', $targ, 1);
    
}



package npc_vindicator_monolith_guardian;
@npc_vindicator_monolith_guardian::ISA = qw( monolith_guardian );
use strict;
use const_stats;
...
sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(  ($self->{'STAT'}->[22] - $targ->{'STAT'}->[21])*5  > 1 ) { return if $self->spell_hgeneric('TSTAB', $targ, 1); }
    $self->attack_melee($targ, $weapon); 
}
...
sub punish_user {
    ...
    if($self->{'ROOM'} != $victim->{'ROOM'}) {
	   ...
	   if ($victim->skill_has(84)){
	   $victim->log_append("Your presence has tricked the guards into not attacking you.\n");
	   return;
       }
         ...
         if(rand(1)<.4  && $victim->{'HP'} > 0) {
           if ($self->{'REC'} == 646) { # davada says: canines growl!
               ...
               rand(1)<.2?
                   ...
                   if(rand(10)<.05){$self->cmd_do("mumble show me the way");}
           }
         }
...
@maiden::rand_idle = (
    sub { $_[0]->say("hear me speek"); }
);

sub on_ask ($topic) {
 my ($self, $topic, $who) = @_;
 if ($topic =~ /god|slab|gods/)  {
      $self->say("my god why have you forsaken me");
      return('');
 } else { return undef; }
 return("");
} 
...
package azral_outcast;
@azral_outcast::ISA = qw( npc );
use strict;

@azral_outcast::rand_idle = (
    sub { $_[0]->room_tell("{3}$_[0]->{'NAME'} dances around the room, filling it with joy!\n"); },
    sub { $_[0]->say("Ask me about the azral vilalge to learn something fun."); },
);

sub on_ask ($topic) {
 my ($self, $topic, $who) = @_;
 if ($topic =~ /stuff/)  {
      $self->say("I know things about the Azral Village that others only wished they knew.");
      return('');
 } elsif ($topic =~ /azral|village|azral village/)  {
      $self->say("$who->{'NAME'}, you want me to tell you the secrets of the Azral Village?");
      return('');
 } elsif ($topic =~ /secrets/)  {
      $self->say("Show me your secrets, I do believe thats what i said.");
      $self->say("Thank you for taking the time to talk to me.");
      return '';
 } else { return undef; }
 return("");
} 

package npc_angry_dragon;
@npc_angry_dragon::ISA = qw( npc_quest_receiver );
use strict;

sub on_ask {
    my ($self, $topic, $from) = @_;
    if ($topic =~ /sad|cry|upset/i) { 
        return <<END_CAP;
The Dragon wipes the tears from her eyes and says, "I am sorry I am making a nuisance of myself, but thieves have stolen something priceless from me"
END_CAP
    } elsif ($topic =~ /stolen|steal|priceless/i) { 
    return <<END_CAP;
Dragon says, "Well the thief could not take my treasure as it is cursed if anyone takes it from my cave, so they took the most precious thing..... my unhatched egg"
END_CAP
    } elsif ($topic =~ /stuff/i) { 
    return <<END_CAP;
Dragon says, "I know things about being sad."
END_CAP
    } elsif ($topic =~ /thief|thieves/i) { 
    return <<END_CAP;
Dragon checks the room for spies and quietly says, "The thief was obscured by a obfuscation spell, however the footprints tell me it was either a small male or a female!"
END_CAP
    return undef;
	}
}
...
package npc_ergin;
@npc_ergin::ISA = qw ( npc_quest_receiver );
use strict;

sub on_ask {
    my ($self, $topic, $from) = @_;
    if ($topic =~ /hammer|anvil|blacksmith/i) { 
        return <<END_CAP;
Ergin says, "I am the only blacksmith in the realm, trained by the Cellarion dwarves who live under the treacherous volcano Atumba"
END_CAP
    } elsif ($topic =~ /shield|guard|make/i) { 
        return <<END_CAP;
Ergin says, "I want to help you but as a blacksmith I cannot give you the secrets of the realm."
END_CAP
    } elsif ($topic =~ /secrets/i) { 
      return <<END_CAP;
Ergin checks the room for spies and quietly says, "The holder of the secrets is very unhappy.....If you follow the wails you will find her"
END_CAP
    return undef;
	}
}
...
package npc_tarma;
@npc_tarma::ISA = qw( incorporeal );
use strict;

sub on_receive {
    my ($self, $obj, $from) = @_;
    if($obj->{'REC'} != 826) { return $self->SUPER::on_receive(@_); }#<---------------she wants the amulet---
    $self->room_sighttell("{2}Tarma's eyes light up as she takes the heriloom.\n");
    $self->say("I can't thank you enough for returning this to me.");
    $obj->dissolve_allsubs(); # trash the amulet  ;) 
    my $item = $self->item_spawn(820); # give dragons egg <--------------- she will give the egg-----------
    $self->item_hgive($item, $from, 0, 1); 
    $self->delay_say(4, "This heirloom will never leave my sight again, I'm glad you have taken that egg actually, it has started making odd noises.");
}

sub on_ask {
    my ($self, $topic, $from) = @_;
    if ($topic =~ /help|secret|secrets|egg|eggs/i) { 
        return <<END_CAP;
Tarma says, "Yes I have something that does not belong to me, but a family heirloom was stolen from me and its irreplaceable."
END_CAP
    } elsif ($topic =~ /heirloom|stolen|find/i) { 
        return <<END_CAP;
Tarma says, "The heirloom that was stolen was a very precious and valuable amulet, passed down through my family for five generations. It happened one very stormy night; it must have been so thunderous and windy that I didn't hear the intruder."
END_CAP
    } elsif ($topic =~ /intruder|amulet/i) { 
        return <<END_CAP;
Tarma says, "Well I suppose if you could get my family heirloom back, I will given you my hidden item."
END_CAP
    return undef;
	} elsif ($topic =~ /stuff/i) { 
        return <<END_CAP;
Tarma says, "I know things about eggs and secrets."
END_CAP
    return undef;
	}
}

package npc_optical_monolith_guardian;
@npc_optical_monolith_guardian::ISA = qw( monolith_guardian );
use strict;
use const_stats;
...
sub attack_sing {
    ...
    if ($victim->{'TYPE'} == 1 && rand(100) < 5 && !$victim->effect_has(68)) {
        $self->room_sighttell("{11}$self->{'NAME'} {1}does something to {11}$victim->{'NAME'}\{1}, and something happens.\n", $victim);
        $victim->log_append("{11}$self->{'NAME'} {1}does something to $self->{'PPOS'} something happens.\n", $victim);
...
package npc_obsidian_protector;
@npc_obsidian_protector::ISA = qw ( npc_quest_receiver );
use strict;
...
package npc_baine;
@npc_baine::ISA = qw ( npc_quest_receiver );
use strict;

sub on_ask {
    my ($self, $topic, $from) = @_;
    if ($topic =~ /help|stuff/i) { 
        return <<END_CAP;
Baine says, "Adamant is not meant for the likes of thee, give it back to me."
END_CAP
    }elsif ($topic =~ /adamant/i) { 
        return <<END_CAP;
Baine says, "I know things about capes, crowns, bracers, and rings."
END_CAP
	}elsif ($topic =~/cape|crown|bracers|ring/i){
	   return <<END_CAP;
Baine says, "Adamant goes in my box, one circle, two arms, three points, four corners"
END_CAP
	}elsif ($topic =~/box/i){
	   return <<END_CAP;
...
sub attack_sing {
  ...
  if($victim->is_dead || $self->is_dead) { return; }
    ...
    if(rand(100)<40) { $self->cmd_do("blind $victim->{'NAME'}"); $self->cmd_do("kill $victim->{'NAME'}");  }
    elsif($self->{'WEAPON'}) { $self->attack_melee($victim, $main::objs->{$self->{'WEAPON'}}); }
    elsif($self->{'DWEAP'}) {  $self->attack_melee($victim, &main::obj_lookup($self->{'DWEAP'})); }
    return;
  }
...
package vampire_bat;
@vampire_bat::ISA = qw( npc );

sub attack_sing {
  my ($self, $victim) = @_;
  if($victim->is_dead || $self->is_dead) { return; }
  else {
    if(rand(100)<10) { $self->orc_suikill2($victim); }
    elsif($self->{'WEAPON'}) { $self->attack_melee($victim, $main::objs->{$self->{'WEAPON'}}); }
    elsif($self->{'DWEAP'}) {  $self->attack_melee($victim, &main::obj_lookup($self->{'DWEAP'})); }
    return;
  }
...
sub orc_suikill2 {
  ...
  if($self->is_dead() || $victim->is_dead() || ($victim->{'ROOM'} != $self->{'ROOM'})) { return; }
  delete $self->{'CRYL'}; # no cryl on a suikill
  my $dam = int($victim->{'MAXH'}/10 - 20 + int rand(41));
  $victim->log_append("{2}The $self->{'NAME'} flies at you and it\'s fangs pierce deep into your neck causing {12}$dam {2}damage.\n");
  $victim->room_sighttell("{2}The $self->{'NAME'} flies towards {12}$victim->{'NAME'}\{2}! and sucks his blood causing {12}$dam {2}damage.\n");
  $victim->{'HP'}-=$dam;
  if($victim->is_dead()) { $victim->die($self); }
  return;
}

package npc_she_wolf;
@npc_she_wolf::ISA = qw(npc);
use strict;
	
	sub on_receive {
	    my ($self, $obj, $from) = @_;
	    if($obj->{'REC'} != 920) { return $self->SUPER::on_receive(@_); }#<---------------she wants the amulet of freedom

	    $self->room_sighttell("{2}She Wolf wears the amulet and transforms back into a beautiful princess.\n");
	    $self->delay_say("Thank you for releasing me from this evil curse");

	    $obj->dissolve_allsubs(); # trash the amulet ;)

	    $self->{'CRYL'}=3000;
	          $self->cryl_give(3000, $from);
	          #$from->quest_add(0); <------------------is this line needed....or does the quest number have to change??
	          # exp reward
	          $from->{'EXPPHY'}+=250000;# <--------------- she will give exp and vanish
      		  $from->log_append("{17}You gain {1}250,000 {17}physical experience.\n"); 

	    # respawn self -- I didn't die, so the onDeath_RESPAWN isn't otherwise executed.
	    $main::eventman->enqueue($self->{'onDeath_RESPAWN'} * 10, \&rockobj::item_spawn, $self->container(), $self->{'REC'});
	    $self->obj_dissolve();
	}
	
sub on_ask {
    my ($self, $topic, $from) = @_;
    if ($topic =~ /secret|hidden/i) { 
        return <<END_CAP;
She Wolf begins to tell her tale, "I have not always been in this form. I was once a princess living in a far off land to the west, when I turned 18 many suitors came to fight for my hand. One of these suitors was a handsome man named David, he won all the contests and we were to be married."
END_CAP
    } elsif ($topic =~ /marriage|david|marry/i) { 
        return <<END_CAP;
She Wolf says, "The night before we were to be married, a mad woman named Adamer burst into my chambers and performed a ritual that made me shape shift into the form you see now."
END_CAP
    } elsif ($topic =~ /shape shift|woman|adamer/i) { 
        return <<END_CAP;
She Wolf says, "You see Adamer had fallen deeply in love with Prince David and although David had declined all advances from the countess, she still felt that they were destined to be together. She transported me to this horrible place, and for many years I heard the sounds of my beloved being tortured to the very edge of his being."
END_CAP
    } elsif ($topic =~ /torture/i) { 
        return <<END_CAP;
She Wolf says, "I have searched for many years to find the door to Adamer\'s chambers but I cannot find the entrance. I did however find a strange key made of human bones"
END_CAP
    return undef;
	}
}

package npc_wwind_caster;
@npc_wwind_caster::ISA = qw( spell_caster );
use strict;
  sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(rand(100) < 40) { return if $self->spell_hgeneric('WWIND', $targ, 1); }
    $self->attack_melee($targ, $weapon); 
}
package npc_blight_caster;
@npc_blight_caster::ISA = qw( spell_caster );
use strict;
  sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(rand(100) < 40) { return if $self->spell_hgeneric('BLIGHT', $targ, 1); }
    $self->attack_melee($targ, $weapon); 
}
package npc_fblast_caster;
@npc_fblast_caster::ISA = qw( spell_caster );
use strict;
  sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(rand(100) < 40) { return if $self->spell_hgeneric('FBLAST', $targ, 1); }
    $self->attack_melee($targ, $weapon); 
}

package pufferfish;
@pufferfish::ISA = qw( npc );
use strict;
...
sub on_room_enter (objects: enterer; string: fromdir, observation) {
  ...
  if(rand(100) < 50) { return; }
  $who->log_append("{2}$self->{'NAME'} {4}punctures your body!\n");
  $who->room_sighttell("{2}$self->{'NAME'} {4}punctures {2}$who->{'NAME'}\'s {4}body!\n");
  $who->effect_add(71);
  return;
}
sub on_itemthrew (item, thrower, fromdir) {
...
package npc_drunken_guard;
@npc_drunken_guard::ISA = qw( npc );
use strict;

sub on_receive {
    my ($self, $obj, $from) = @_;
    if($obj->{'REC'} != 138) { return; }
    else{
    $obj->dissolve_allsubs(); # trash the amulet  ;) 
    if(!$main::map->[$self->{'ROOM'}]->{$self->{'TRIGEXIT'}}->[0]) { return; }
   	...
   	$self->say("ii Can' haNk yOoouhhuh ehnnnooouhgh ffoR GivInNng mmE thIiiZ CoooFfeeh.");
   	$self->room_sighttell("The gate to the north opens wide.\n");
...
sub on_ask {
    ...
    if ($topic =~ /secret/i) { 
        return <<END_CAP;
Drunken Guard says, "mmY sSsSZshift iSsS aLImoSsSt Ooos0noFaer, ahnd i ne*hic*d taOoo ssobehr up. GOoo any sjooofffee?\n
END_CAP
    } elsif ($topic =~ /gate/i) { 
        return <<END_CAP;
Drunken Guard says, "I n*hic*Ed caoffffehe To aoPehn theh gate.\n
END_CAP
    } elsif ($topic =~ /coffee/i) { 
        return <<END_CAP;
Drunken Guard says, "yEESsS CooOofFfffeh isSzz What i weeeaN.n\n
END_CAP
    } elsif ($topic =~ /stuff/i) { 
        return <<END_CAP;
Drunken Guard says, "mmY sSsSZshift iSsS aLImoSsSt Ooos0noFaer, ahnd i ne*hic*d taOoo ssobehr up. GOoo any sjooofffee?\n
...
package npc_spider_queen;
@npc_spider_queen::ISA = qw( npc );
use strict;
...
sub on_receive {
  ...
  if($obj->{'REC'}==980) { 
	  	  $self->say("Ohhhh thank you i love treasures, it will look very pretty in my nest!\nHave you given me the other two objects to complete this set?");
  }elsif($obj->{'REC'}==981){
		  $self->say("Ohhhh thank you i love treasures, it will look very pretty in my nest!\nHave you given me the other two objects to complete this set?");
  }elsif($obj->{'REC'}==982){
		  $self->say("Ohhhh thank you i love treasures, it will look very pretty in my nest!\nHave you given me the other two objects to complete this set?");
	  ...
	  $obj->obj_dissolve();
	  $self->say("'Thats not a very attractive object for my nest, perhaps you should find some better things.");
	  return;
  }
  my $hasall = 0;
  if($self->inv_rec_scan(980) && $self->inv_rec_scan(981) && $self->inv_rec_scan(982)) {
      $self->say('Yes that will do nicely, I will do as you ask.');
      $self->cmd_do("giggle");
      $from->item_spawn(983);
      $self->say('There you go, now run along before i change my mind.');
      foreach my $obj ($self->inv_objs) {$obj->obj_dissolve(); }
      $from->{'EXPMEN'}+=1_500_000; 
      $from->log_append("{17}You gain {1}1,500,000 {17}mental experience.\n"); 
...
package beholder;
@beholder::ISA = qw( spell_caster );
use strict;

sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(  ($self->{'STAT'}->[22] - $targ->{'STAT'}->[21])*5  > 600 ) { return if $self->spell_hgeneric('MBLAST', $targ, 1); }
    $self->attack_melee($targ, $weapon); 
}
package npc_tslave;
@npc_tslave::ISA = qw( npc );
use strict;

sub on_die {
 	my ($self, $by) = @_;
	&main::rock_shout(undef, "{11}$self->{'NAME'} has been released from his agony by $by->{'NAME'}!\n");
}

package npc_kerel;
@npc_kerel::ISA = qw( npc );
use strict;
...
sub on_ask {
    ...
    if ($topic =~ /secret/i) { 
        return <<END_CAP;
Ker\'el says, "I seem to be missing the latest additions to my tapestries."
END_CAP
    } elsif ($topic =~ /case|crystal|tapestry/i) { 
        return <<END_CAP;
Ker\'el says, "Collect my missing tapestries, bring them back, and put them in the case."
END_CAP
    } elsif ($topic =~ /halo/i) { 
        return <<END_CAP;
Ker\'el says, "This halo is a reward to those whom are faithful."
END_CAP
    } elsif ($topic =~ /stuff/i) { 
        return <<END_CAP;
Ker\'el says, "I seem to be missing the latest additions to my tapestries."
...
package npc_sacred_priest;
@npc_sacred_priest::ISA = qw( npc );

sub on_attack {
  my ($self, $attacker, $victim, $weapon) = @_;
  if($self == $victim && (.5 < rand 1)) { 
     $self->{'HP'}=$self->{'MAXH'};
     $self->room_sighttell("{3}$self->{'NAME'} is surrounded by a healing aura.\n");
  }
  return $self->SUPER::on_attack(@_);
}

package npc_tidal_wave_caster;
@npc_tidal_wave_caster::ISA = qw( spell_caster );
use strict;
  sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(rand(100) < 40) { return if $self->spell_hgeneric('TWAVE', $targ, 1); }
    $self->attack_melee($targ, $weapon); 
}

##*************added by Gill
package npc_helpford;
@npc_helpford::ISA = qw( npc ); # note!! USED TO check object *BEFORE* player object.
use strict;
...
sub on_ask ($topic) {
    ...
    if($topic =~ /combat/) { $self->say_rand('Aaaaa the age old art of combat are you physical or are you just mental.', 'Hey manky pants dont make me teach you a lesson!.', 'Ooo Arr matey...my cousin the pirate loves to fight......but then again he is Vile!');  }
    if($topic =~ /physical/) { $self->say('The trick to physical combat is strength, speed and knowledge');  }
    if($topic =~ /mental/) { $self->say('Mental combat can be a tricky business, greatly aided by improving skills with one of the many courses from the academy');  }
    if($topic =~ /academy|school|courses/) { $self->say('Ooo yes the academy is where smart players go to learn. Hmm lets see from here you would take the following directions: w,s,go gate,n,se,go gate');  }
    if($topic =~ /store|buy|mall|shopping/) { $self->say('Well clucky the magnificent lies to the south of here and she holds a wide range of fine items. For a wider selection try Bob\'s');  }
    if($topic =~ /clucky/) { $self->say('************I will tell about clucky when i am ready....dont push me ******************');  }
    if($topic =~ /bob|mall|commerce/) { $self->say('Bob\'s commercial district has a large selection of stores all selling top quality merchandise for any adventurer. To get to Bob\'s from here go ....ummm ...w, s, se, d, ne, e, e');  }
    if($topic =~ /training/) { $self->say('************I will tell about training when i am ready....dont push me ******************');  }
    elsif ($topic =~ /weapon|\send/) {
        my $item = $self->item_spawn(65); # give shortsword
    	if($self->item_hgive($item, $from, 0, 0)){ 
        $self->say_rand('Now dont lose this one, I don\'t have too many left...no sirree Bob!', 'Don\'t tell anyone about this or I will be struck off the board of special dudes.', 'Remember a shortsword is not just for Christmas its for the whole reset!!!'); 
		}
    }
    else { return undef; }
    return("");
...
package npc_greatsword_quest;
@npc_greatsword_quest::ISA = qw( npc );
use strict;

sub on_receive {
    my ($self, $obj, $from) = @_;
        if($obj->{'REC'} != 1008) { return $self->SUPER::on_receive(@_); }
    if($from->{'CRYL'} >= 15000) { 
        $from->{'CRYL'} -= 15000;
        $self->room_sighttell("{2}Di-Kuzan takes the offered Large Rpugh Onyx piece and 15000 cryl. With a wave of his hand, and a few muttered words, he fashions the piece of Onyx into a Greatsword.\n");
        my $new_item = $self->item_spawn(1009);
        $obj->dissolve_allsubs(); # get rid of the zeode; people were killing melal to get it back. meanies.
        $self->item_hgive($new_item, $from, 0, 1); 
    } else {
        $self->item_hgive($obj, $from, 0, 1); 
        $self->say('If you want me to make you a sword, it will cost 15000 cryl.');
    }
}

sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
    if($topic =~ /stuff/) { $self->say('I have knowledge of Great onyx swords.');  }
    elsif($topic =~ /sword|swords|onyx/) { $self->say('I can make a truly wonderous sword for thee if thou can retrieve a large piece of onyx and pay for the time.');  }
    else { return undef; }
    return("");
}

package npc_city_protector;
@npc_city_protector::ISA = qw( npc );

sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
}

package npc_haste_amulet;
@npc_haste_amulet::ISA = qw( npc );
...
package npc_eldor;
@npc_eldor::ISA = qw( npc ); # note!! USED TO check object *BEFORE* player object.
use strict;

sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
    if($topic =~ /training/) { $self->say('Before I can teach you how to fight you must ensure that you are healthy, read the health notice in this room for further assistance with this.'); }
    elsif($topic =~ /combat/) { $self->say('Attack the chickens and the cockrels get the password and move on. When you find sadine ask her about "stuff".'); }
    elsif($topic =~ /exit/) { $self->say('To exit from this area say "exitme"');}
} 

package npc_cockrell;
@npc_cockrell::ISA = qw( npc ); 

sub on_die {
  # is called right before dying
  my $self = shift;
  $self->room_talktell($self->{'DEATHMSG'});
  return;
}

package npc_sadine;
@npc_sadine::ISA = qw( npc ); # note!! USED TO check object *BEFORE* player object.
use strict;

sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
    if($topic =~ /spells|stuff/) { $self->say('Yes I can teach you about mental combat if you would like, just ask me about mindmelt, and firebolt');}
    elsif($topic =~ /mindmelt/) { 
	    $self->say('You have learned to cast mindmelt');
    	$from->skill_add(81);}
    elsif($topic =~ /firebolt/) { 
	    $self->say('You have learned to cast firebolt');
    	$from->skill_add(85);}
}

package npc_ducky;
@npc_ducky::ISA = qw( npc ); # note!! USED TO check object *BEFORE* player object.
use strict;


sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
    if($topic =~ /training|farm|newbie/) { $self->say('Yeah this facility was developed solely to train new rockaholics. Here you will learn the basics of combat both mental and physical as well as a few hints and tips to help you on your journey through this troubled land'); }
    elsif($topic =~ /trouble|problems/) { $self->say('The problems of this land started many eons ago, now the races all fight to gain control on the monoliths of power scattered throughout the land');}
    elsif($topic =~ /clucky|wife|love/) { $self->say('It pains me to tell my story......');}
    elsif($topic =~ /story|pain/) { $self->say('The love of my life is a barkeeper at the Plucky Tavern, we were married in the olden days by a Crimson Priest but alas we were cursed by Plat the mighty mage and can no longer be together');}
    elsif($topic =~ /plat/) { $self->say('Plat is the root of all wisdom');}
    elsif($topic =~ /morbis/) { $self->say('Morbis is the mighty seer of all things');}
    elsif($topic =~ /foamer/) { $self->say('Foamer is the ruler of all that is foamy');}
    elsif($topic =~ /aura/) { $self->say('Aura is queen of kisses');}
} ;

package farmyard_cook;
@farmyard_cook::ISA = qw( npc );
use strict;

sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
    if($topic =~ /recipie/) { $self->say('A big juicy apple would do nicely.'); }
    elsif($topic =~ /reward/) { $self->say('Hmm well I have some of that cryl stuff lying around here somewhere"');}
}
...
sub on_idle {
  ...
  if(rand(10)>5) { return; }
  SWITCH:for(int rand(2)) {
    /0/ && do { $self->room_sighttell("{3}The $self->{'NAME'} Oooo I need fruit for my recipie.\n"); last SWITCH; };
    /1/ && do { $self->room_sighttell("{3}With a sigh, the $self->{'NAME'} says, Can you get me an apple o brave adventurer\n"); last SWITCH; };
  }
  return;
}
...
sub on_receive {
  my ($self, $obj, $from) = @_;
  if($obj->{'REC'}==1051 && !$from->{'COOK_QUEST'} ) {
    $self->room_talktell("{17}\"wonderful.. take this as payment.\"\n");
   {
      $self->{'CRYL'}=200;
      $self->cryl_give(200, $from);
      # exp reward
      $from->{'EXPMEN'}+=500; 
      $from->log_append("{17}You gain {1}500 {17}mental experience.\n"); 
    }
    $from->{'COOK_QUEST'}=1;
    # respawn self -- I didn't die, so the onDeath_RESPAWN isn't otherwise executed.
    $main::eventman->enqueue($self->{'onDeath_RESPAWN'} * 30, \&rockobj::item_spawn, $self->container(), $self->{'REC'});
    $self->dissolve_allsubs;
  }
  $obj->dissolve_allsubs;
  return;
...
package npc_stallion;
@npc_stallion::ISA = qw( npc ); 
...
package npc_tsunami_caster;
@npc_tsunami_caster::ISA = qw( spell_caster );
use strict;
  sub sc_gen_attack {
    my ($self, $targ, $weapon) = @_;
    if(rand(100) < 10) { return if $self->spell_hgeneric('TSUN', $targ, 1); }
    $targ->log_append("{11}$self->{'NAME'} unleashes a mighty tsunami across the land!\n");
    $self->attack_melee($targ, $weapon); 
}

package npc_green_elf;
@npc_green_elf::ISA = qw( npc ); # note!! USED TO check object *BEFORE* player object.
use strict;

sub on_ask ($topic) {
    my ($self, $topic, $from) = @_;
    if($topic =~ /quest/) { $self->say('There are meny quests, you just have to look for them.');  }
    elsif($topic =~ /vrean/) { $self->say('The Vreans are a magnificent race of beings which evolved from the arachnids, making them agile and strong.');  }
    elsif($topic =~ /secret/) { $self->say('There is a shrine to Albyrith hidden in this area.');  }
    elsif($topic =~ /albyrith/) { $self->say('Hmm. the next trial master may have more information on this.');  }
    elsif($topic =~ /dryne|spectrite|taer|shi-kul/) { $self->say('Keep them out from here at all cost! They only want to steal our secrets.');  }
    elsif($topic =~ /learned one/) { $self->say('He is by far the most wise of all the Vreans. He has studied the origins of the race and some say that he holds a distinct resemblance to Albyrith, and knows all the ancient skills of this great race.');  }
    elsif($topic =~ /elf|elves/) { $self->say('We protect the wildlife and and greenery from trespassers and ensure that on Vreans of a worthy aptitude are permitted onto the next stage of the trial.');  }
    elsif($topic =~ /help/) { $self->say('No one ever wants to do anything themselves anymore!');  }
    elsif($topic =~ /agility/) { $self->say_rand('The Vreans have a natural aptitude for increased agility and speed.', 'There are many usefull skills and items which will aid your mobility.', 'Remember that Vreans are lower to the ground and therefore more importance should be placed on lower agility.');  }
    elsif($topic =~ /pw|password|passwords/) { $self->say('You have to prove yourself first.');  }
    else { return undef; }
} ;

package npc_amber_elf;
@npc_amber_elf::ISA = qw( npc ); # note!! USED TO check object *BEFORE* player object.
use strict;
...
sub on_ask ($topic) {
    ...
    elsif($topic =~ /shrine/) { $self->say('The shrine is located near the shaded grove.');  }
    elsif($topic =~ /albyrith/) { $self->say('She is the founder of the Vreans, as told in the scribes of old.');  }
    elsif($topic =~ /dryne|spectrite|taer|shi-kul/) { $self->say('Keep them out from here at all cost! They only want to steal our secrets.');  }
    elsif($topic =~ /learned one/) { $self->say('He is located somewhere in this area, and he holds many secrets about this area.');  }
    elsif($topic =~ /elf|elves/) { $self->say('We protect the wildlife and and greenery from trespassers and ensure that only Vreans of a worthy aptitude are permitted onto the next stage of the trial.');  }
    elsif($topic =~ /help/) { $self->say('Bah!');  }
    elsif($topic =~ /agility/) { $self->say_rand('The Vreans have a natural aptitude for increased agility and speed.', 'There are many usefull skills and items which will aid your mobility.', 'Remember that Vreans are lower to the ground and therefore more importance should be placed on lower agility.');  }
    elsif($topic =~ /pw|password|passwords/) { $self->say('You have to prove yourself first.');  }
    else { return undef; }
}
...
package npc_benzal_traveler;
@npc_benzal_traveler::ISA = qw( npc );
use strict;

@npc_benzal_traveler::rand_idle = (
    sub { $_[0]->say("Ask me about stuff to learn something fun."); },
);

sub on_ask ($topic) {
 my ($self, $topic, $who) = @_;
 if ($topic =~ /stuff/)  {
      $self->say("I know things about the Spherule Factory, Benzal Eldar, Benzal Master Orb-smith, and this jungle.");
      return('');
 } elsif ($topic =~ /spherule|factory|spherule factory/)  {
      $self->say("$who->{'NAME'}, the Spherule Factory is ancient factory that my people have long since kept hidden deep in these jungles. There are is an entrance to the northwest of the azral village. The Azral\'s are our protectors but have stolen a great artifact from us.");
      return('');
 } elsif ($topic =~ /benzal eldar|eldar/)  {
      $self->say("The Benziliane Eldar keeps his office in the western part of the factory, to get to him you must seek the eldar, believe in things which you can not see.  The eldar knows the history of our people as he has seen the tests of time.");
      return '';
 } elsif ($topic =~ /master orb-smith|master|orb-smith/)  {
      $self->say("The Benziliane Master orb-smith is skilled in orb crafting, he is responsible for all the orbs that you see, I have heard rumors of equipement made from orb dust, you should ask him about it.");
      return '';
 } elsif ($topic =~ /jungle/)  {
      $self->say("This jungle is a mysterious place, there are many secrets here. At one point in time the Benzal Eldar told me stories of a secret back entrance to the factory. When i was younger i found it and nearly got lost in the maze. I found my way out by when i found words etched on a wall in the maze then went through the backdoor at the north.  There used to be a ladder leading out from under a rock in the jungle, but that ladder has long been broken. I suppose it could be repaired with a metal bar if you could find one.");
      return '';
 } else { return undef; }
 return("");
} 

package npc_benzal_eldar;
@npc_benzal_eldar::ISA = qw( npc );
use strict;

@npc_benzal_eldar::rand_idle = (
    sub { $_[0]->say("Ask me about stuff to learn something fun."); },
);
...
sub on_ask ($topic) {
 my ($self, $topic, $who) = @_;
 if ($topic =~ /stuff/)  {
      $self->say("I know things about this factory, and many other stories of my people.");
      return('');
 } elsif ($topic =~ /spherule|factory|spherule factory/)  {
      $self->say("This factory is filled with many secrets, the master orb-smith is hidden to the west he will be able to tell you about the latest and greatest that this factory has to offer and many new developments in orb technology.");
      return('');
 } elsif ($topic =~ /stories/)  {
      $self->say("The benzal\'s have been protected for many years by the azral\'s. As payment for their services we have given them orbs, and made the equipment that they carry very powerful. One day many many years ago the high priest stole a box of my figures. Little did he know the true powers of it. When combined with a massive stick, and the most powerful of all orbs the figure could be wieled. The resulting staff of time acts as a powerful orb with many unknown and mysterious abilities. If you could retrive the staff and the forementioned pieces you could borrow the figure from me.");
      return '';
 } else { return undef; }
 return("");
} 
...
#npc_eldar_koalite
package incorporeal_npc_eldar_koalite;
@incorporeal_npc_eldar_koalite::ISA = qw( incorporeal );
use strict;

@incorporeal_npc_eldar_koalite::rand_idle = (
    sub { $_[0]->say("Ask me about stuff to learn something fun."); },
);
...
sub on_ask ($topic) {
 my ($self, $topic, $who) = @_;
 if ($topic =~ /stuff/)  {
      $self->say("I know everything there is to know about orbs.");
      return('');
 } elsif ($topic =~ /orbs/)  {
	$self->say("My craft has become obsolete, a machine does what i used to do. I hear noises below me all through the night, and the new orbs and equipment no longer have my seal of approval. The process is now automated, just put the dust into the machine and out comes shiny new equipment but becareful how you touch it.");
      return '';
 } else { return undef; }
 return("");
}
...
#npc_old_man
package incorporeal_npc_old_man;
@incorporeal_npc_old_man::ISA = qw( incorporeal );
use strict;

@incorporeal_npc_old_man::rand_idle = (
    sub { $_[0]->say("Ask me about stuff to learn something fun."); },
);
...
#npc_kelzly_the_widow
package npc_kelzly_the_widow;
@npc_kelzly_the_widow::ISA = qw( npc );
use strict;

@npc_kelzly_the_widow::rand_idle = (
    sub { $_[0]->say("Ask me about stuff to learn something fun."); },
);
...
package npc_benzal_master;
@npc_benzal_master::ISA = qw( npc );
use strict;
...
package npc_giant_centipede;
@npc_giant_centipede::ISA = qw( npc );
use strict;
...
sub on_take {
    ...
    if(.95 > rand(1)) {
        # scurry away 90% of the time
		if($self->{'HIDDEN'}) { #checks to see if invis
        	$self->{'HIDDEN'}--; #makes visible again
        }#endif (invis)
        	$by->room_sighttell("{3}Giant Centipede speeds away as $self->{'NAME'} tries to catch it!\n");
        	$by->log_append("{3}Giant Centipded runs away at speed as you try to catch it!\n");
        	$by->inv_del($self);
        	$main::map->[$by->{'ROOM'}]->inv_add($self);
        	$self->auto_move(1);
    }#endif (scurry)
...
sub on_idle {
    my ($self) = @_;
    if(.65 > rand(1)) {
        # will go invis 65% of the time
        $_[0]->room_talktell("{3}Giant Centipede vanishes from sight!\n");
        $self->{'HIDDEN'} = 1 unless ($self->{'HIDDEN'}>1);;
    }
}

package npc_trimoral;
@npc_trimoral::ISA = qw( npc );
use strict;

sub on_room_enter {
    my ($self, $who, $dir) = @_;
    if($who->{'RACE'} != $self->{'RACE'} && rand 1 < .1 && !$who->inv_rec_scan(349) ) {
        $who->room_sighttell("{2}$self->{'NAME'} touches their trimoral triangle and $who->{'NAME'}is blinded!\n");
        $who->log_append("{12}$self->{'NAME'} touches their trimoral triangle and you become blind\n");
        $who->effect_add(22);
    }
    return &npc::on_room_enter(@_);
}
...
sub on_die {
  ...
  &main::kill_all_socks("THE SERVER IS RESETTING. Try coming back later (the reset will only take a couple seconds, or an hour). Sorry for any inconvenience but someone killed Reset.\n");
  ...
  foreach $uid (@files) {
    if ($uid =~ /(.+?)\.r2/) {
       $fname = "$main::base_code_dir/saved/".lc($uid);
        unlink($fname);  # (cuz we dont want fists, now, do we? :-)) (we dont want blank ones either)
    }
   }
   
  opendir(DIR, 'iop') || die "Cannot open iop-file directory: $!\n";
  my @files = readdir(DIR); rewinddir(DIR); closedir(DIR);
  my ($uid, @user_objs, @cap);
  my $time = time;
  my $fname;
  foreach $uid (@files) {
    if ($uid =~ /(.+?)\.r2/) {
       $fname = "$main::base_code_dir/iop/".lc($uid);
        unlink($fname);  # (cuz we dont want fists, now, do we? :-)) (we dont want blank ones either)
    }
   }
   
   unlink("main::base_code_dir/dbs/ips_resolved.r2");
   unlink("main::base_code_dir/dbs/ips_connected.r2");
   unlink("main::base_code_dir/dbs/general_votes.r2");
   unlink("main::base_code_dir/dbs/help.r2");
   unlink("main::base_code_dir/dbs/obj_limits.r2");
   unlink("main::base_code_dir/dbs/obj_recd.r2");
   unlink("main::base_code_dir/dbs/obj_unique.r2");
   unlink("main::base_code_dir/dbs/player_bounties.r2");
   unlink("main::base_code_dir/dbs/recnum_toname.r2");
   unlink("main::base_code_dir/dbs/rock_mdim.db");
   unlink("main::base_code_dir/dbs/rock_stats.r2");
   unlink("main::base_code_dir/dbs/ips_resolved.r2");
