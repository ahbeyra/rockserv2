...
package item_killol_crate;
@item_killol_crate::ISA = qw( item );
use strict;

sub on_use {
  my ($self, $who) = @_;
  return if !$who;
  $who->log_append("{17}You pull a killol from the crate.\n");
  $who->room_sighttell("{17}$who->{'NAME'} pulls a killol from the crate.\n");
  $who->item_spawn(5);
  return;
}
